...
pub trait PolicyAgent: Clone + Send + Sync + 'static {
    ...
    /// Every emitted event still passes [`Self::check_event`] as defense in
    /// depth. Registration uses the atomic commit lane, so a constituent
    /// denial aborts the batch before persistence and leaves the catalog
    /// unchanged. The default allows.
