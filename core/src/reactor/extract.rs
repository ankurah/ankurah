...
use ankurah_proto::{EntityId, PropertyId, PropertyPath};
...
/// [`Value::Json`] to match how literals are stored in the comparison index
/// after TypeResolver.
pub(crate) fn extract_property_value<E: AbstractEntity>(id: &PropertyId, sub_path: &[String], entity: &E) -> Option<Value> {
    let root_value = match id {
        PropertyId::EntityId(id) => E::value_by_id(entity, EntityId::from_ulid(*id)),
        // The `id` pseudo-property reads by its reserved name.
        PropertyId::Id => E::value(entity, "id"),
        PropertyId::System { name } => E::value(entity, name),
    }?;
