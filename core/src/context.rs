...
pub trait TContext {
    ...
    fn check_write(&self, entity: &Entity, model: &crate::ModelId) -> Result<(), AccessDenied>;
...
impl<SE: StorageEngine + Send + Sync + 'static, PA: PolicyAgent + Send + Sync + 'static> TContext for NodeAndContext<SE, PA> {
    ...
    fn check_write(&self, entity: &Entity, model: &crate::ModelId) -> Result<(), AccessDenied> {
        self.node.policy_agent.check_write(&self.cdata, model, entity, None)
    }
    async fn get_entity(&self, id: proto::EntityId, collection: &crate::ModelId, cached: bool) -> Result<Entity, RetrievalError> {
    ...
    async fn commit_local_trx(&self, trx: &Transaction) -> Result<Vec<Event>, MutationError> {
        ...
        // Defensively recheck staged values against the catalog now that every
        // schema used by this transaction is admitted. Typed accessors already
        // canonicalize before staging; this covers low-level/internal writes
        // before they can become events.
        ...
        for entity in trx.entities.iter() {
            let model = trx
                .entity_model(&entity.id)
                .ok_or_else(|| MutationError::General(format!("transaction omitted model projection for entity {}", entity.id).into()))?;
            if let Some(event) = entity.generate_commit_event()? {
                ...
                if matches!(model, proto::ModelId::System(_)) {
                    return Err(MutationError::General(format!("model '{model}' is protected and not writable by transactions").into()));
                }
                ...
                entity_events.push((model, entity.clone(), event));
            }
        }
        ...
        for (model, entity, event) in entity_events {
            ...
            let memberships = forked.memberships();
            let mut memberships = memberships.iter().copied();
            let Some(canonical_model) = memberships.next() else {
                return Err(MutationError::InvalidUpdate("event must result in exactly one canonical membership"));
            };
            if memberships.next().is_some() {
                return Err(MutationError::InvalidUpdate("event must result in exactly one canonical membership"));
            }
            if canonical_model != model {
                return Err(MutationError::InvalidUpdate("event membership does not match the transaction's model projection"));
            }
            let attestation = self.node.policy_agent.check_event(&self.node, &self.cdata, &model, &entity_before, &forked, &event)?;
            let attested = Attested::opt(event.clone(), attestation);

            attested_events.push(attested.clone());
            entity_attested_events.push((model, entity, attested));
        }
        ...
        for (_, entity, attested_event) in &entity_attested_events {
        ...
        for (_model, entity, attested_event) in entity_attested_events {
            ...
            intents.push(ResidentWriteIntent::from_events(canonical_entity, vec![attested_event])?);
        }
        let event_getter = crate::retrieval::LocalEventGetter::new(self.node.storage.clone(), self.node.durable);
        ...
        for result in commit.entities {
            ...
            if result.canonical_changed {
                for model in result.materialized_as {
                    changes.push(EntityChange::new(model, entity.clone(), events.clone())?);
                }
            } else {
    ...
    PA: PolicyAgent + Send + Sync + 'static,
{
    ...
    pub(crate) async fn get_entity(&self, collection_id: &ModelId, id: proto::EntityId, cached: bool) -> Result<Entity, RetrievalError> {
        ...
        if let Some(local) = self.node.entities.get(&id) {
            if !local.has_membership(collection_id) {
        ...
        match self.node.storage.get_state(id).await {
            Ok(entity_state) => {
                if !entity_state.payload.state.memberships.contains(collection_id) {
                    return Err(RetrievalError::EntityNotFound(id));
                }
