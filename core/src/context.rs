    ...
    async fn commit_local_trx(&self, trx: &Transaction) -> Result<Vec<Event>, MutationError> {
        ...
        for entity in trx.entities.iter() {
            ...
            if let Some((genesis, schema)) = genesis_events.get(&entity.id) {
                ...
                // (commit_remote_transaction). The genesis carries the
                // membership assertion, judged against the compiled
                // declaration create() held -- descriptor cells first, local
                // catalog second -- so an ephemeral's first write does not
                // wait on its catalog subscription.
                node.check_membership_admissibility(Some(schema), genesis)?;
            ...
            if let Some(event) = entity.generate_commit_event(proto::AuthorId::Unknown)? {
                node.check_membership_admissibility(None, &event)?;
                events.push(event);
            }
