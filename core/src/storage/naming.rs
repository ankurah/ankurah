...
/// Every widening candidate through the full 32-character hexadecimal id
/// suffix is taken. Effectively unreachable (the full suffix colliding means
/// another identity already claimed this id's own full-suffix name), so it is
/// refused honestly rather than returning a name known to be taken, which
/// would silently alias two identities onto one physical column.
...
/// The trailing lowercase hexadecimal characters used to disambiguate
/// physical names.
///
/// Hexadecimal is deliberately used instead of lowercased base64: lowercasing
/// base64 would destroy identity information, while preserving its case would
/// leak mixed-case identifiers back into otherwise normalized schemas.
fn id_suffix(id: &EntityId, len: usize) -> String {
    let hex = id.to_bytes().iter().map(|byte| format!("{byte:02x}")).collect::<String>();
...
pub fn dedupe(desired: &str, id: &EntityId, is_taken: impl Fn(&str) -> bool) -> Result<String, NamingExhausted> {
    ...
    // The len == 32 candidate was already tested taken above; returning it
    // anyway would alias. Refuse instead.
    Err(NamingExhausted { desired: desired.to_string(), last: format!("{}_{}", desired, id_suffix(id, 32)) })
}
...
pub fn fallback(prefix: &str, id: &EntityId, is_taken: impl Fn(&str) -> bool) -> Result<String, NamingExhausted> {
    ...
    Err(NamingExhausted { desired: prefix.to_string(), last: format!("{}_{}", prefix, id_suffix(id, 32)) })
}
...
mod tests {
    ...
    fn dedupe_suffixes_with_trailing_id_chars_and_widens() {
        let property = id(1);
        let hex = property.to_bytes().iter().map(|byte| format!("{byte:02x}")).collect::<String>();
        let suffix4 = hex[hex.len() - 4..].to_string();
