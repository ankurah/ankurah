...
/// String-specific helpers for engine-facing property paths.
///
/// These split a dotted path into `key` and `sub_path`, or join those parts
/// back together. A resolved `IndexKeyPart<PropertyId>` has no string form
/// to parse or reconstruct.
...
impl<K: PartialEq> KeySpec<K> {
    ...
    pub fn matches(&self, other: &KeySpec<K>) -> Option<IndexSpecMatch> {
        ...
        for (self_keypart, other_keypart) in self.keyparts.iter().zip(other.keyparts.iter()) {
            if self_keypart.key != other_keypart.key || self_keypart.sub_path != other_keypart.sub_path {
                return None;
            }
