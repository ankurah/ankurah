...
/// The node's resident entities: at most one live `EntityInner` per entity ID.
///
/// Every `Entity::Resident` handle for an ID shares that one instance, so a change applied to it
/// reaches every View and livequery holding it. The set holds only weak references; an entity stays
/// resident while something uses it and is loaded from storage again when next needed.
///
/// Only committed state enters: `get_or_retrieve` and `with_state` load entities from storage or a
/// peer's answer, and `publish_new` registers a creation once its storage transaction commits.
/// Pending creations stay outside. Every resident entity takes the set's system epoch.
#[derive(Clone)]
...
impl WeakEntitySet {
    ...
    pub fn get(&self, id: &EntityId) -> Option<Entity> {
        self.entities.read().unwrap().get(id)?.upgrade().map(Entity::Resident)
    }
    ...
    pub fn conjure_evil_phantom(&self, id: EntityId, model: ankurah_proto::ModelId) -> Entity {
        ...
        Entity::Resident(self.get_or_insert_state(id, &state).unwrap().1)
    }
            ...
            None => {
                if let Some(stored) = state_getter.get_state(id).await? {
                    ...
                    match self.get_or_insert_state(id, &state)? {
                        (true, entity) => entity,
                        (false, entity) => return Ok((None, Entity::Resident(entity))),
                    }
                }
            }
        };
        let result = entity.state.apply_state(event_getter, &state).await?;
        Ok((Some(matches!(result, StateApplyResult::Applied)), Entity::Resident(entity)))
    }
}
...
mod tests {
    ...
    fn resident_and_temporary_values_preserve_property_ids_and_missing_values() -> anyhow::Result<()> {
        ...
        let entity = Entity::Resident(entities.get_or_insert_state(EntityId::from_bytes([1; 32]), &state.to_state()?)?.1);
    ...
    #[test]
    fn forks_retain_their_resident_and_nodes_keep_independent_instances() -> anyhow::Result<()> {
        ...
        let resident = Entity::Resident(original.get_or_insert_state(id, &state)?.1);
        let fresh = Entity::Resident(replacement.get_or_insert_state(id, &state)?.1);
        ...
        drop(replacement);
        assert_eq!(fresh.resident()?.registry.get(&id), Some(fresh.clone()));
        drop(fresh);
        assert!(registry.upgrade().is_none(), "the registry must not form a cycle with its resident entities");
        Ok(())
    }
}
