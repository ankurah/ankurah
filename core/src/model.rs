...
pub trait Model: Sized {
    ...
    /// The compiled registration declaration for this Rust model: its
    /// source-level model label and the per-field
    /// `(name, backend, value_type, optional)` descriptors from
    ...
    fn initialize_new_entity(&self, entity: &Entity, model: &crate::ModelId) -> Result<(), MutationError>;
}
...
pub trait Mutable {
    ...
    fn read(&self) -> Result<Self::View, PropertyError> {
        ...
        Self::View::from_entity(new_inner, self.model_id())
    }
}
