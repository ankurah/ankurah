...
impl NodeApplier {
    fn require_state_membership(model: &proto::ModelId, state: &proto::State) -> Result<(), MutationError> {
        if state.sole_membership() != Some(*model) {
            return Err(MutationError::InvalidUpdate("received state does not have exactly the routed model membership"));
        }
        Ok(())
    }

    fn require_event_membership(model: &proto::ModelId, event: &proto::Event) -> Result<(), MutationError> {
        let explicit: Vec<_> = event
            .operations
            .memberships()
        ...
        if event.is_entity_create() {
            if explicit.as_slice() != [*model] {
                return Err(MutationError::InvalidUpdate("genesis event membership does not match its routed model"));
            }
        ...
        E: SuspenseEvents + Send + Sync,
    {
        ...
        for fragment in event_fragments {
            ...
            Self::require_event_membership(&model, &attested_event.payload)?;
        ...
        match content {
            ...
            proto::UpdateContent::EventOnly(event_fragments) => {
                ...
                if !applied_events.is_empty() {
                    let saved = Self::save_state(node, &entity, applied_events.clone(), event_getter).await?;
                    changes.extend(Self::changes_after_save(&entity, applied_events, saved)?);
                } else if failure.is_none() {
                    let saved = Self::save_state(node, &entity, Vec::new(), event_getter).await?;
            ...
            proto::UpdateContent::StateAndEvent(state_fragment, event_fragments) => {
                ...
                Self::require_state_membership(&model, &state.payload.state)?;
                ...
                if matches!(changed, Some(true) | None) {
                    ...
                    let saved = Self::save_state(node, &entity, attested_events.clone(), event_getter).await?;
    ...
    {
        let result = node
            .commit_resident_writes(vec![ResidentWriteIntent::from_resident_state(entity.clone(), events)?], None, event_getter)
            .await?;
    ...
    fn changes_for_models(
        ...
        models.into_iter().map(|model| EntityChange::new(model, entity.clone(), events.clone())).collect()
    }
        ...
        S: GetState + Send + Sync,
    {
        ...
        match content {
            proto::DeltaContent::StateSnapshot { state } => {
                let attested_state: Attested<proto::EntityState> = (entity_id, state).into();
                Self::require_state_membership(&collection_id, &attested_state.payload.state)?;
            ...
            proto::DeltaContent::EventBridge { events } => {
                ...
                // Save updated state
                let saved = Self::save_state(node, &entity, attested_events, event_getter).await?;
