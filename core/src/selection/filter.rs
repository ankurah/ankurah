...
fn evaluate_expr<S: Stage, I: ValueLookup<S>>(item: &I, expr: &Expr<S>) -> Result<ExprOutput<Value>, Error> {
    match expr {
...
pub fn evaluate_predicate<S: Stage, I: ValueLookup<S>>(item: &I, predicate: &Predicate<S>) -> Result<bool, Error> {
    match predicate {
