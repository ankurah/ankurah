...
/// to the canonical state identified by `expected_head`. Model-backed
/// memberships are part of canonical state; engines derive their private
/// membership maps and materializations from `state`.
#[derive(Debug, Clone)]
...
impl PreparedEntityWrite {
    ...
    pub fn new(expected_head: Clock, state: Attested<EntityState>) -> Self { Self { expected_head, state } }
}
