...
use ankurah_proto::{Clock, EntityId, Event, EventId, Membership, ModelId, Operation, OperationSet};
...
/// through a node's catalog, so any stable id works. Genesis membership is
/// part of the content-hashed event, so every fixture uses this same value.
...
fn make_test_event(seed: u8, parent_ids: &[EventId]) -> Event {
    ...
    Event { entity_id, parent: Clock::from(parent_ids.to_vec()), operations: OperationSet::default() }
}
...
/// Like `make_lww_event`, but with an explicit parent clock for multi-layer
/// DAG scenarios.
fn make_lww_event_with_parent(seed: u8, properties: Vec<(&str, &str)>, parent_ids: &[EventId]) -> Event {
    ...
    let mut operations = OperationSet::from_backends(BTreeMap::from([("lww".to_string(), ops)]));
    if parent_ids.is_empty() {
        operations.push(Operation::Membership(Membership::Add(test_model_id())));
    }
    Event { entity_id, parent: Clock::from(parent_ids.to_vec()), operations }
}
...
mod yrs_layer_tests {
    ...
    fn make_yrs_event(seed: u8, text_field: &str, insert_text: &str) -> Event {
        ...
        Event { entity_id, parent: Clock::default(), operations: OperationSet::from_backends(BTreeMap::from([("yrs".to_string(), ops)])) }
    }
...
mod entity_change_batches {
    ...
    fn lww_event_for(entity_id: EntityId, properties: Vec<(&str, &str)>, parent_ids: &[EventId]) -> Event {
        ...
        Event { entity_id, operations, parent: Clock::from(parent_ids.to_vec()) }
    }
    ...
    async fn notification_accepts_batch_superseded_ancestors() {
        ...
        let change = EntityChange::new(test_model_id(), entity.clone(), batch);
        ...
        let bad = EntityChange::new(test_model_id(), entity, vec![Attested::opt(stray, None)]);
