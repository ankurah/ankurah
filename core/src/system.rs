    ...
    PA: PolicyAgent + Send + Sync + 'static,
{
    ...
    pub async fn create(&self) -> Result<()> {
        ...
        match self
            .0
            .storage
            .commit_batch(StorageWriteBatch::new(vec![PreparedEntityWrite::new(
                proto::Clock::default(),
                attested_state.clone(),
                [collection_id],
            )]))
            .await?
        {
    ...
    pub async fn join_system(&self, state: Attested<EntityState>) -> Result<(), MutationError> {
        ...
        match self
            .0
            .storage
            .commit_batch(StorageWriteBatch::new(vec![PreparedEntityWrite::new(expected_head, state.clone(), [collection_id])]))
            .await?
        {
            CommitBatchOutcome::Committed(_) => {}
