...
use crate::storage::{CommitBatchOutcome, PreparedEntityWrite, StorageEngine, StorageWriteBatch};
...
{
    pub(crate) fn new(storage: Arc<SE>, entities: WeakEntitySet, reactor: Reactor, durable: bool) -> Self {
    ...
    pub async fn create(&self) -> Result<()> {
        ...
        let collection_id = ModelId::System(proto::SystemModel::System);
        ...
        lww_backend.set(PropertyId::System(ankql::ast::SystemProperty::Item), proto::sys::Item::SysRoot.into_value()?);

        let event = system_entity.generate_commit_event()?.ok_or(anyhow!("Expected event"))?;

        // Stage the event, apply, then commit
        let event_getter = LocalEventGetter::new(self.0.storage.clone(), true);
        ...
        self.0.storage.append_events(std::slice::from_ref(&attested_event)).await?;
        ...
        match self
            .0
            .storage
            .commit_batch(StorageWriteBatch::new(vec![PreparedEntityWrite::new(proto::Clock::default(), attested_state.clone())]))
            .await?
        {
            CommitBatchOutcome::Committed(_) => {}
            CommitBatchOutcome::Conflict { .. } => {
                return Err(anyhow!("system root was created concurrently"));
            }
        }
    ...
    pub async fn join_system(&self, state: Attested<EntityState>) -> Result<(), MutationError> {
        ...
        let expected_head = match self.0.storage.get_state(state.payload.entity_id).await {
            Ok(current) => current.payload.state.head,
            Err(crate::error::RetrievalError::EntityNotFound(_)) => proto::Clock::default(),
            Err(error) => return Err(error.into()),
        };
        match self.0.storage.commit_batch(StorageWriteBatch::new(vec![PreparedEntityWrite::new(expected_head, state.clone())])).await? {
            CommitBatchOutcome::Committed(_) => {}
            CommitBatchOutcome::Conflict { .. } => return Err(MutationError::TOCTOUAttemptsExhausted),
        }
    ...
    /// Reset all engine-owned canonical data, model associations, and
    /// materializations.
    ...
    async fn hard_reset_locked(&self) -> Result<()> {
        ...
        *self.0.system_ready.write().unwrap() = false;
        ...
        // Delete all application data from storage. Even a failed deletion must
        // release the catalog's resetting latch so callers are not stranded;
        // system/reactor clearing remains success-only.
        if let Err(error) = self.0.storage.delete_all().await {
            if let Some(hook) = &catalog_reset_hook {
                (hook.finish)();
            }
    ...
    async fn load_system_catalog(&self) -> Result<()> {
        ...
        let state_getter = LocalStateGetter::new(self.0.storage.clone());
        let event_getter = LocalEventGetter::new(self.0.storage.clone(), self.0.durable);

        for state in self
            .0
            .storage
            .fetch_states(&collection_id, &ankql::ast::Selection { predicate: ankql::ast::Predicate::True, order_by: None, limit: None })
            .await?
        {
            ...
            if let Some(value) = lww_backend.get(&PropertyId::System(ankql::ast::SystemProperty::Item)) {
