...
pub struct Content<CD: ContextData> {
    ...
    pub context_data: CD,
    pub version: u32,
    /// Optional validity of the subsystem that owns this query. Catalog setup,
    /// for example, supplies its current setup generation so a response from
    /// an earlier generation cannot install schema after reset. Ordinary user
    /// queries have no separate owner lifecycle and leave this `None`.
    ///
    /// Each remote attempt combines this with the relay's own
    /// `request_generation`; both must still be current before a response or
    /// streaming update may mutate local state.
    pub owner_validity: Option<RequestValidity>,
}
...
impl<CD: ContextData, Q: RemoteQuerySubscriber> SubscriptionRelay<CD, Q> {
    ...
    pub(crate) fn subscribe_query_with_validity(
        ...
        debug!("SubscriptionRelay.subscribe_predicate() - New predicate {} needs remote registration", query_id);
        {
            self.inner.subscriptions.lock().expect("poisoned lock").insert(
                query_id,
                RemoteQueryState {
                    content: Arc::new(Content { collection_id, selection, context_data, query_id, version, owner_validity }),
            ...
            match subscriptions.get_mut(&query_id) {
                Some(state) => {
                    ...
                    state.content = Arc::new(Content {
                        ...
                        version,
                        owner_validity: old_content.owner_validity.clone(),
                    });
                    ...
                    match state.status {
                        Status::Established(peer_id, _old_version) => {
                            ...
                            Some((
                                ...
                                state.content.owner_validity.clone(),
                            ))
    ...
    pub(crate) async fn validities_for_stream_update(
        &self,
        peer_id: &proto::EntityId,
        ...
        let candidates: Vec<(proto::ModelId, Vec<(ModelId, Option<RequestValidity>)>)> = {
                ...
                .map(|item| {
                    let owners = item
                        ...
                        .filter_map(|query_id| {
                            ...
                            (current_peer == *peer_id).then(|| (state.content.collection_id.clone(), state.content.owner_validity.clone()))
                        })
                        .collect();
                    (item.model.clone(), owners)
                })
                .collect()
        };

        let mut validities = Vec::new();
        for (item_model, item_candidates) in candidates {
            let mut candidate = None;
            for (collection, validity) in item_candidates {
                if collection == item_model && validity.as_ref().is_none_or(RequestValidity::is_current) {
                    candidate = Some(validity);
                    break;
                }
            }
            ...
            match subscriptions.get(&query_id) {
                ...
                Some(info) => {
                    let same_attempt = info.request_generation == request_generation
                        && info.content.version == version
                        && matches!(info.status, Status::Requested(current_peer, current_version) if current_peer == peer_id && current_version == version);
                    same_attempt && info.content.owner_validity.as_ref().is_some_and(|validity| !validity.is_current())
                }
            }
        };
        if cleanup {
    ...
    pub fn collection_for_query(&self, query_id: proto::QueryId) -> Option<ModelId> {
        let subscriptions = self.inner.subscriptions.lock().unwrap_or_else(|e| e.into_inner());
    ...
    async fn attempt_subscribe(
        ...
        let validity = self.request_validity(query_id, target_peer, version, request_generation, content.owner_validity.clone());
        match node
    ...
    PA: crate::policy::PolicyAgent + Send + Sync + 'static,
{
    async fn remote_subscribe(
        ...
        if deltas.iter().any(|delta| delta.model != model) {
            return Err(RetrievalError::Other("QuerySubscribed response crossed the requested model boundary".to_owned()));
        }
        ...
        let event_getter = crate::retrieval::CachedEventGetter::new(collection_id, &node, context_data);
...
mod tests {
    ...
    struct MockMessageSender<CD: ContextData> {
        ...
        sent_requests: Arc<Mutex<Vec<(EntityId, proto::QueryId, ModelId, ankql::ast::Selection)>>>,
    ...
    impl<CD: ContextData> MockMessageSender<CD> {
        ...
        fn get_sent_requests(&self) -> Vec<(EntityId, proto::QueryId, ModelId, ankql::ast::Selection)> {
    ...
    fn test_model_id(byte: u8) -> ModelId {
    ...
    #[async_trait]
    impl TNode<ModelId> for BlockingMessageSender {
        async fn remote_subscribe(
            &self,
            _peer_id: EntityId,
    ...
    fn establish_for_peer(
        relay: &SubscriptionRelay<ModelId, MockLiveQuery>,
        peer_id: EntityId,
        query_id: proto::QueryId,
        validity: Option<RequestValidity>,
    ) {
        let _ = relay.set_node(Arc::new(MockMessageSender::<ModelId>::new()));
    ...
    async fn stream_admission_requires_a_current_source_for_every_item() {
        ...
        assert!(matches!(relay.validities_for_stream_update(&peer_id, &honest).await, Some(validities) if validities.is_empty()));

        let unknown = proto::QueryId::new();
        let partly_stale = [stream_item(vec![first]), stream_item(vec![unknown])];
        assert!(relay.validities_for_stream_update(&peer_id, &partly_stale).await.is_none());
        assert!(relay.validities_for_stream_update(&peer_id, &[]).await.is_none());

        let other_peer = EntityId::new();
        assert!(relay.validities_for_stream_update(&other_peer, &[stream_item(vec![first])]).await.is_none());

        let mut wrong_model = stream_item(vec![first]);
        wrong_model.model = proto::ModelId::System(proto::SystemModel::Property);
        assert!(relay.validities_for_stream_update(&peer_id, &[wrong_model]).await.is_none());
    }
    ...
    async fn stream_admission_rejects_an_invalidated_owner_before_application() {
        ...
        let validities = relay.validities_for_stream_update(&peer_id, &items).await.expect("current owner admits its stream");
        assert_eq!(validities.len(), 1);
        drop(validities.into_iter().next().unwrap().try_acquire().expect("current fence grants a lease"));

        fence.invalidate();
        assert!(relay.validities_for_stream_update(&peer_id, &items).await.is_none());
    }
