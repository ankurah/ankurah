...
where
    ...
    PA: PolicyAgent + Send + Sync + 'static,
{
    let source_queries = item.source_queries;
    if source_queries.is_empty() {
        tracing::debug!("Dropping reactor update for peer {} because it has no owning query", peer_id);
        return None;
    }
    ...
    Some(proto::SubscriptionUpdateItem { entity_id: item.entity.id(), model: item.model, content, predicate_relevance, source_queries })
}
