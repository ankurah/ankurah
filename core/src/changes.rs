...
where I: View
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ItemChange::Initial { item } => {
                write!(f, "Initial {}/{}", <I::Model as crate::model::Model>::descriptor().label, item.id())
            }
            ItemChange::Add { item, .. } => {
                write!(f, "Add {}/{}", <I::Model as crate::model::Model>::descriptor().label, item.id())
            }
            ItemChange::Update { item, .. } => {
                write!(f, "Update {}/{}", <I::Model as crate::model::Model>::descriptor().label, item.id())
            }
            ItemChange::Remove { item, .. } => {
                write!(f, "Remove {}/{}", <I::Model as crate::model::Model>::descriptor().label, item.id())
            }
        }
    }
}
