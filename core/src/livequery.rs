...
struct Inner {
    ...
    pub(crate) collection_id: ModelId,
    ...
    PA: PolicyAgent + Send + Sync + 'static,
{
    ...
    // Resolution is idempotent and performs origin-side literal validation at
    // each site. Every reactor/storage consumer still re-casts at execution.
    ...
    if args.cached || !has_relay {
        ...
        crate::task::spawn(async move {
            ...
            let resolved = match resolve_node.upgrade() {
                Some(node) => node.resolve_selection_names(Some(&resolve_cdata), &resolve_collection, schema, &current).await,
                None => {
...
impl EntityLiveQuery {
    /// Model projection selected by this query.
    pub(crate) fn model_id(&self) -> ModelId { self.0.collection_id }

    pub fn new<SE, PA>(node: &Node<SE, PA>, collection_id: ModelId, args: MatchArgs, cdata: PA::ContextData) -> Result<Self, RetrievalError>
    where
        SE: StorageEngine + Send + Sync + 'static,
        PA: PolicyAgent + Send + Sync + 'static,
    {
...
impl<R: View> LiveQuery<R> {
    ...
    pub fn resultset(&self) -> ResultSet<R> { self.0 .0.resultset.wrap::<R>(self.0 .0.collection_id) }
...
impl<R: View + Clone + 'static> Get<Vec<R>> for LiveQuery<R> {
    fn get(&self) -> Vec<R> {
        ...
        self.0 .0.resultset.wrap::<R>(self.0 .0.collection_id).peek()
    }
}

// Implement Peek trait - delegate to ResultSet<R>
impl<R: View + Clone + 'static> Peek<Vec<R>> for LiveQuery<R> {
    fn peek(&self) -> Vec<R> { self.0 .0.resultset.wrap(self.0 .0.collection_id).peek() }
}
...
where R: Clone + Send + Sync + 'static
{
    ...
    where L: IntoSubscribeListener<ChangeSet<R>> {
        ...
        self.0 .0.subscription.subscribe(move |reactor_update: ReactorUpdate| {
            let changeset: ChangeSet<R> = livequery_change_set_from(me.0 .0.resultset.wrap::<R>(me.0 .0.collection_id), reactor_update);
            listener(changeset);
        })
    }
}
...
mod tests {
    ...
    fn test_model_id(byte: u8) -> ModelId {
        ...
        bytes[0] = byte;
        ModelId::EntityId(proto::EntityId::from_bytes(bytes))
    }
    ...
    async fn remote_activation_error_releases_initialization_waiters() {
        ...
        let query = EntityLiveQuery(Arc::new(Inner {
            ...
            collection_id: test_model_id(0x71),
            schema: None,
    ...
    async fn update_selection_wait_surfaces_the_latched_error() {
        ...
        let query = EntityLiveQuery(Arc::new(Inner {
            ...
            current_version: std::sync::atomic::AtomicU32::new(1),
            selection: Mut::new((ankql::ast::Selection { predicate: ankql::ast::Predicate::True, order_by: None, limit: None }, 1)),
            collection_id: test_model_id(0x72),
            schema: None,
            gap_fetcher: Arc::new(EmptyGapFetcher),
        }));
