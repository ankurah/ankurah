...
impl Transaction {
    ...
    pub async fn get<'rec, 'trx: 'rec, M: Model>(&'trx self, id: &EntityId) -> Result<MutableBorrow<'rec, M::Mutable>, RetrievalError> {
        ...
        // Fetching does not mint identity, so a failed registration only
        // warns here: property accessors resolve against the catalog at
        // construction, so a field the catalog does not bind refuses its
        // write at set() with PropertyError::UnknownProperty.
        if let Err(e) = self.dyncontext.ensure_registered(M::schema()).await {
            tracing::warn!("registration unavailable on fetch-to-edit for '{}'; unbound fields will refuse writes: {}", M::collection(), e);
        }
