...
impl LWWBackend {
    ...
    fn apply_operations_internal(&self, operations: &[BackendOperation], event_id: Option<EventId>) -> Result<(), MutationError> {
...
impl PropertyBackend for LWWBackend {
    ...
    fn to_operations(&self) -> Result<Option<Vec<BackendOperation>>, MutationError> {
        ...
        Ok(Some(vec![BackendOperation { diff }]))
    }
    ...
    fn unknown_diff_version_is_refused() {
        ...
        let bad_diff =
            bincode::serialize(&LWWDiff { version: 3, data: bincode::serialize(&BTreeMap::<PropertyId, Option<Value>>::new()).unwrap() })
                .unwrap();
        let err = backend.apply_operations(&[BackendOperation { diff: bad_diff }]).unwrap_err();
