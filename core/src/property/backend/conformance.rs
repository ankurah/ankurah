...
/// A system-keyed [`PropertyId`] for these backend-level conformance tests:
/// they exercise a backend directly (no catalog, no entity), so every
/// property here is addressed by name, same as a system/catalog collection.
