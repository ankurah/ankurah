...
impl PropertyBackend for YrsBackend {
    ...
    fn properties(&self) -> Vec<PropertyId> {
        ...
        trx.root_refs().filter_map(|(name, _)| name.parse::<PropertyId>().ok()).filter(|property| *property != PropertyId::Id).collect()
    }
    ...
    fn from_state_buffer(state_buffer: &Vec<u8>) -> std::result::Result<Self, crate::error::RetrievalError> {
        ...
        let update =
            yrs::Update::decode_v2(state_buffer).map_err(|e| crate::error::RetrievalError::FailedUpdate(std::sync::Arc::new(e)))?;
        txn.apply_update(update).map_err(|e| crate::error::RetrievalError::FailedUpdate(std::sync::Arc::new(e)))?;
...
#[cfg(test)]
mod tests {
    use ankurah_proto::SystemProperty;

    use super::*;

    #[test]
    fn system_property_roots_are_enumerated() {
        let backend = YrsBackend::new();
        let property = PropertyId::System(SystemProperty::Label);
        backend.insert(&property, 0, "value").unwrap();

        assert_eq!(backend.property_values().get(&property), Some(&Some(Value::String("value".into()))));
    }
}
