...
impl<Projected> YrsString<Projected> {
    pub fn new(property_id: PropertyId, backend: Arc<YrsBackend>, entity: Entity) -> Self {
        Self { property_id, backend, entity, phantom: PhantomData }
    }

    pub fn value(&self) -> Option<String> { self.backend.get_string(&self.property_id) }
    ...
    pub fn insert(&self, index: u32, value: &str) -> Result<(), MutationError> {
        ...
        self.backend.insert(&self.property_id, index, value)
    }
    pub fn delete(&self, index: u32, length: u32) -> Result<(), MutationError> {
        if !self.entity.is_writable() {
            return Err(PropertyError::TransactionClosed.into());
        }
        self.backend.delete(&self.property_id, index, length)
    }
    pub fn overwrite(&self, start: u32, length: u32, value: &str) -> Result<(), MutationError> {
        if !self.entity.is_writable() {
            return Err(PropertyError::TransactionClosed.into());
        }
        self.backend.delete(&self.property_id, start, length)?;
        self.backend.insert(&self.property_id, start, value)?;
        Ok(())
    }
    pub fn replace(&self, value: &str) -> Result<(), MutationError> {
        ...
        let length = self.backend.get_string(&self.property_id).unwrap_or_default().len() as u32;
        self.backend.delete(&self.property_id, 0, length)?;
        self.backend.insert(&self.property_id, 0, value)?;
        Ok(())
    }
}
...
impl<Projected> FromEntity for YrsString<Projected> {
    fn from_entity(property_id: PropertyId, entity: &Entity) -> Self {
        ...
        Self::new(property_id, backend, entity.clone())
    }
}
...
impl<Projected> InitializeWith<String> for YrsString<Projected> {
    fn initialize_with(entity: &Entity, property_id: PropertyId, value: &String) -> Result<Self, MutationError> {
        let new_string = Self::from_entity(property_id, entity);
