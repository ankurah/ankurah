...
pub struct YrsString<Projected> {
    ...
    /// marks an unbound field (`insert`/`delete`/`value`-via-`try_value` then
    /// surface `PropertyError::UnknownProperty`).
...
impl<Projected> YrsString<Projected> {
    pub fn new(property_name: PropertyName, backend: Arc<YrsBackend>, entity: Entity) -> Self {
        ...
        // override.
        let property_id = entity.resolve_property_id(&property_name).ok();
        Self { property_name, property_id, backend, entity, phantom: PhantomData }
    }
    fn new_addressed(property: PropertyAddress, backend: Arc<YrsBackend>, entity: Entity) -> Self {
        ...
        Self { property_name: property.name, property_id, backend, entity, phantom: PhantomData }
    }
