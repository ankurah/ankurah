...
pub struct LWW<T: Property> {
    ...
    /// [`PropertyId::System`]. `None` means resolution failed; an unknown name
    /// is reported as `PropertyError::UnknownProperty`, while
    /// `resolution_error` retains a more specific resolver failure such as an
    /// ambiguous admitted alias. The backend keys on this value alone and
    /// never resolves: this is the one and only resolution hop.
...
impl<T: Property> FromEntity for LWW<T> {
    fn from_entity(property: PropertyAddress, entity: &Entity) -> Self {
        ...
        // and unbound -> Err(UnknownProperty) branches. Retain non-unknown
        // failures so ambiguity does not degrade into a later unknown-field
        // error.
        ...
        Self { property_name: property.name, property_id, resolution_error, backend, entity: entity.clone(), phantom: PhantomData }
    }
}
