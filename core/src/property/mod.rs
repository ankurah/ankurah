...
/// Resolve a derive-generated field before constructing its active accessor.
///
/// The generated view/mutable owns this registration-label-to-identity hop.
/// Property accessors and backends receive only the resulting durable
/// [`PropertyId`]. An explicit `#[property(id = "...")]` binding bypasses the
/// catalog lookup.
#[doc(hidden)]
pub fn resolve_property_id(
    entity: &crate::entity::Entity,
    model: &crate::ModelId,
    name: &str,
    explicit_id: Option<ankurah_proto::EntityId>,
) -> Result<PropertyId, PropertyError> {
    match explicit_id {
        Some(id) => Ok(PropertyId::EntityId(id)),
        None => entity.resolve_property_id(model, name),
    }
}
...
pub(crate) fn name_keyed(
    ...
    values
        ...
        .filter_map(|(key, value)| match key {
            PropertyId::System(property) => Some((property.to_string(), value)),
...
pub trait Property: Sized {
    ...
    /// [`Self::from_value`] by the compiled projection when the backend holds no
    /// entry (see [`read_by_id`] and
