...
//!
//! Policy gates the execution at two complementary boundaries (RFC 5.7):
...
use ankurah_proto::{
    self as proto, Attested, EntityId, MembershipDescriptor, ModelDescriptor, PropertyDescriptor, PropertyRef, RegisteredMembership,
    RegisteredModel, RegisteredProperty, TransactionId,
};
    ...
    PA: PolicyAgent + Send + Sync + 'static,
{
    ...
    pub async fn execute_schema_registration(
        ...
        // check; request authentication and the resolved RegistrationPlan are
        // the registration policy boundaries.
        {
            ...
            for collection in &named {
                if collection.starts_with(crate::schema::RESERVED_COLLECTION_PREFIX) {
                    return Err(RegistrationError::ReservedCollection(collection.to_string()));
                }
            }
        }
        ...
        let mut events: Vec<proto::ModelContext<Attested<proto::Event>>> = Vec::new();
        let mut push = |event: proto::ModelContext<proto::Event>| {
            events.push(proto::ModelContext::new(event.model, Attested::opt(event.value, None)));
        };

        // -- models: looked up by source label -------------------------------
            ...
            let (model_id, resolved_name) = match m.explicit_id {
                ...
                None => match self.model_lookup_checked(&m.collection).await? {
                    Some(def) => {
                        ...
                        if def.name != m.name {
                            let (_, head) = self
                                .catalog_entity_snapshot(def.id)
                                .await?
                    ...
                    None => {
                        ...
                        push(creation(
                            model_collection(),
                            id,
                            vec![("label", Value::String(m.collection.clone())), ("name", Value::String(m.name.clone()))],
                        ));
                        (id, m.name.clone())
                    }
                },
                    ...
                    None => match self.model_lookup_checked(c).await? {
                        ...
                        None => {
                            ...
                            push(creation(
                                model_collection(),
                                id,
                                vec![("label", Value::String(c.to_string())), ("name", Value::String(c.to_string()))],
                            ));
        ...
        for ms in &memberships {
            ...
            let property_id = match &ms.property {
                PropertyRef::Id(id) => {
                    ...
                    if self.catalog_entity_values(*id).await?.is_none() {
    ...
    async fn verify_explicit_binding(
        ...
        let Some(values) = self.catalog_entity_values(id).await? else {
            return Err(RegistrationError::ExplicitIdNotFound { property: id });
        };
    ...
    async fn verify_explicit_model_binding(
        ...
        let Some((values, _)) = self.catalog_entity_snapshot(id).await? else {
            return Err(RegistrationError::ExplicitModelIdNotFound { model: id });
        };
        let found_label = string_field(&values, "label").unwrap_or_default();
        if found_label != m.collection {
            return Err(RegistrationError::ExplicitModelIdMismatch { model: id, found_label, label: m.collection.clone() });
        }
        Ok(values)
    }
    ...
    async fn catalog_entity_values(&self, id: EntityId) -> Result<Option<BTreeMap<String, Option<Value>>>, RetrievalError> {
        Ok(self.catalog_entity_snapshot(id).await?.map(|(values, _)| values))
    }
}
...
fn creation(collection: ModelId, entity_id: EntityId, fields: Vec<(&str, Value)>) -> proto::ModelContext<proto::Event> {
...
) -> proto::ModelContext<proto::Event> {
...
fn follow_up_patch(
    ...
    proto::ModelContext::new(
        collection,
        proto::Event { entity_id, operations: proto::OperationSet(BTreeMap::from([("lww".to_string(), operations)])), parent },
    )
}
