...
// straight from storage (`catalog_row_by_key`), bypassing
// `Resolver::resolve_selection` because it is what bootstraps the catalog that
// resolution would read. A storage engine can only address a property by
