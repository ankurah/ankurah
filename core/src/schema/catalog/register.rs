...
impl CatalogManager {
    ...
    /// its membership set, mint ids for misses, write the differences through
    /// one transaction, and return the resolved definition -- each property
    /// echoing its request entry's `build_id`. Serves both the RegisterSchema
    /// RPC and local first use on the durable node.
    pub async fn register_schema<SE, PA>(
        &self,
        node: &Node<SE, PA>,
        cdata: &PA::ContextData,
        model: RegisterModel,
    ) -> Result<RegisteredModel, RegistrationError>
    where
        ...
        PA: PolicyAgent + Send + Sync + 'static,
    {
        ...
        // to gate and nothing to commit -- but the requester still gets the
        ...
        PA: PolicyAgent + Send + Sync + 'static,
    {
        ...
        if node.durable {
            let model = self.register_schema(node, cdata, request).await?;
            return bind_registered(schema, &model, epoch);
        }
...
fn bind_registered(schema: &'static ModelStructDescriptor, model: &RegisteredModel, epoch: SchemaEpoch) -> Result<(), RegistrationError> {
    let incomplete = || {
        RegistrationError::Retrieval(RetrievalError::Other(format!(
