...
pub trait CatalogResolver: Send + Sync {
    ...
    /// which every engine handles as a fixed primary-key projection. An
    /// implementation whose metadata is incomplete must return an error, not
    /// an empty list: empty means the complete model intentionally projects
