...
use ankurah_proto::{BackendOperation, Clock, EntityId, EntityState, Event, EventId, Membership, Operation, OperationSet, State};
...
/// One canonical in-memory entity, independent of the models through which it
/// is projected.
///
/// Model context belongs to typed views, transactions, queries, and policy
/// calls. Durable model-backed classification lives in [`State::memberships`].
#[derive(Debug, Clone)]
pub struct Entity {
...
struct EntityInnerState {
    ...
    ///
    /// This is transient transaction/ingress intent, not canonical state. It
    /// is cleared when the genesis event applies.
...
/// Enforce the protocol's deliberately narrow membership transition:
/// exactly one add in genesis, and no membership operation thereafter.
fn event_membership(event: &Event) -> Result<Option<ModelId>, MutationError> {
    let memberships: Vec<ModelId> = event
        .operations
        .memberships()
        .map(|membership| match membership {
            Membership::Add(model) => *model,
        })
        .collect();
    if event.is_entity_create() {
        match memberships.as_slice() {
            [model] => Ok(Some(*model)),
            [] => Err(MutationError::InvalidUpdate("genesis event must add exactly one membership")),
            _ => Err(MutationError::InvalidUpdate("genesis event cannot add more than one membership")),
        }
    } else if memberships.is_empty() {
        Ok(None)
    } else {
        Err(MutationError::InvalidUpdate("membership changes after genesis are not supported"))
    }
}
...
fn validate_persisted_memberships(state: &State) -> Result<(), RetrievalError> {
    if state.head.is_empty() {
        ...
        return Err(RetrievalError::Other("empty-head entity state cannot carry memberships".to_owned()));
    }
    if state.memberships.len() != 1 {
        return Err(RetrievalError::Other(format!(
            "persisted entity state must carry exactly one membership, found {}",
...
impl WeakEntity {
    /// Upgrade this handle while the canonical resident entity is still live.
    pub fn upgrade(&self) -> Option<Entity> { self.inner.upgrade().map(|inner| Entity { inner }) }
}
...
impl EntityInner {
    ...
    fn resolve_key(&self, model: &ModelId, name: &str) -> Option<PropertyId> {
        if matches!(model, ModelId::System(_)) {
            return ankql::ast::SystemProperty::from_name(name).map(PropertyId::System);
        }
    ...
    /// Cast a value to the catalog's canonical type for `property`.
    ///
    /// Typed accessors call this before staging a write. The commit path calls
    /// it again for low-level/internal staging paths, so event generation is
    /// also a defensive type boundary.
    pub(crate) fn canonicalize_property_value(
        &self,
        property: &PropertyId,
        value: Value,
    ) -> Result<Value, crate::property::traits::PropertyError> {
        let PropertyId::EntityId(_) = property else { return Ok(value) };
        let Some(resolver) = self.resolver() else { return Ok(value) };
        let target = resolver.property_value_type(property).map_err(|error| {
            crate::property::traits::PropertyError::RetrievalError(crate::error::RetrievalError::Other(error.to_string()))
        })?;
    ...
    /// Read an unresolved system property. Runtime user-model predicates are
    /// resolved to durable property ids before evaluation, so canonical
    /// entities need no model context for this fallback path.
    fn read_system_lenient(&self, name: &str) -> Option<Value> {
        let key = PropertyId::System(ankql::ast::SystemProperty::from_name(name)?);
        let state = self.state.read().expect("other thread panicked, panic here too");
        state.backends.values().find_map(|backend| backend.entry(&key)).flatten()
    }
...
impl Entity {
    ...
    fn weak(&self) -> WeakEntity { WeakEntity { inner: Arc::downgrade(&self.inner) } }

    /// Resolve a source property name through an explicit view model.
    pub(crate) fn resolve_property_id(&self, model: &ModelId, name: &str) -> Result<PropertyId, crate::property::traits::PropertyError> {
        self.inner.resolve_property_id(model, name)
    }
    ...
    /// Whether this entity is durably available through `model`.
    pub fn has_membership(&self, model: &ModelId) -> bool { self.state.read().unwrap().memberships.contains(model) }

    /// Defensively recheck staged values against each property's canonical
    /// value type before event generation. Typed accessors already cast before
    /// staging; this second boundary covers low-level/internal backend writes.
    /// Every key is already a resolved `PropertyId`, so this never re-keys.
    pub(crate) fn canonicalize_pending_values(&self) -> Result<(), crate::property::traits::PropertyError> {
        ...
        for backend in state.backends.values() {
            for key in backend.uncommitted_keys() {
                ...
                // Recheck through the generic trait primitives (`entry` /
                // `restage`). A backend whose edits are not value-shaped (a
                // text CRDT) stages nothing; a tombstone has no value to cast.
                let staged = PropertyId::EntityId(ulid);
                if let Some(Some(value)) = backend.entry(&staged) {
                    let cast = self.canonicalize_property_value(&staged, value)?;
    ...
    /// Construct a new, writable entity with a pending genesis membership.
    ///
    /// Used by generated model creation. The membership enters canonical
    /// state only when event generation emits the corresponding explicit
    /// [`ankurah_proto::Membership::Add`] operation.
    pub fn create(id: EntityId, genesis_membership: ModelId) -> Self {
        Self {
            inner: Arc::new(EntityInner {
                ...
                state: std::sync::RwLock::new(EntityInnerState {
                    ...
                    pending_genesis_membership: Some(genesis_membership),
    ...
    fn from_state(id: EntityId, state: &State) -> Result<Self, RetrievalError> {
        validate_persisted_memberships(state)?;
    ...
    pub(crate) fn generate_commit_event(&self) -> Result<Option<Event>, MutationError> {
        let state = self.state.read().expect("other thread panicked, panic here too");
        let mut backends = BTreeMap::<String, Vec<BackendOperation>>::new();
        for (name, backend) in &state.backends {
    ...
    where E: GetEvents + Send + Sync {
        ...
        let membership = event_membership(event)?;
        ...
        if event.is_entity_create() {
            ...
            if state.head.is_empty() {
                ...
                let model = membership.expect("validated genesis event has one membership");
                if let Some(expected) = state.pending_genesis_membership {
                    if expected != model {
                ...
                state.pending_genesis_membership = None;
    ...
    where E: GetEvents + Send + Sync {
        validate_persisted_memberships(state).map_err(|error| MutationError::General(error.to_string().into()))?;
        let mut head = self.head();
    ...
    /// genesis state.
    pub(crate) fn detached_at_state(
        &self,
        state: Option<&State>,
        replay_genesis_membership: Option<ModelId>,
    ) -> Result<Self, RetrievalError> {
        let detached = match state {
            Some(state) => Self::from_state(self.id, state)?,
            None => {
                let pending = self
                    .state
                    .read()
                    .expect("other thread panicked, panic here too")
                    .pending_genesis_membership
                    .or(replay_genesis_membership)
                    .ok_or_else(|| RetrievalError::Other("missing genesis membership for absent durable entity".to_owned()))?;
                Self::create(self.id, pending)
            }
        };
    ...
    where E: GetEvents + Send + Sync {
        let _guard = self.reconciliation.lock().await;
        {
            ...
            if current.head.is_empty() {
                ...
                current.memberships = state.memberships.clone();
...
impl AbstractEntity for Entity {
    ...
    fn value(&self, field: &str) -> Option<crate::value::Value> {
        if field == "id" {
            ...
            self.inner.read_system_lenient(field)
        }
    }
...
impl Filterable for Entity {
    ...
    fn value(&self, name: &str) -> Option<Value> {
        if name == "id" {
            ...
            self.inner.read_system_lenient(name)
        }
    }
...
impl WeakEntitySet {
    fn require_membership(id: EntityId, model: &ModelId, state: &State) -> Result<(), RetrievalError> {
        if !state.head.is_empty() && !state.memberships.contains(model) {
        ...
        E: GetEvents + Send + Sync,
    {
        ...
        match self.get(id) {
            Some(entity) => {
                if !entity.head().is_empty() && !entity.has_membership(collection_id) {
                    return Err(RetrievalError::EntityNotFound(*id));
                }
                Ok(Some(entity))
            }
            None => match state_getter.get_state(*id).await? {
                None => Ok(None),
                Some(state) => {
                    Self::require_membership(*id, collection_id, &state.payload.state)?;
    ...
    fn private_get_or_create(&self, id: EntityId, collection_id: &ModelId, state: &State) -> Result<(bool, Entity), RetrievalError> {
        Self::require_membership(id, collection_id, state)?;
        let mut entities = self.0.entities.write().unwrap();
        ...
        let entity = Entity::from_state(id, state)?;
        entities.insert(id, entity.weak());
        Ok((false, entity))
    }
...
mod tests {
    ...
    #[test]
    fn resident_entity_is_canonical_across_model_lookups() {
        let entities = WeakEntitySet::default();
        let model_a = test_model_id();
        let entity_a = entities.create(model_a);
        let entity_b = entities.get(&entity_a.id()).expect("canonical entity remains resident");

        assert!(Arc::ptr_eq(&entity_a.inner, &entity_b.inner), "model handles must share canonical state");
        assert_eq!(entity_a, entity_b, "resident lookup returns the same canonical entity");
    }
    ...
    fn system_key_tombstone_shadows_later_backend_value() {
        ...
        assert_eq!(
            entity.read_lenient(&ModelId::System(crate::proto::SystemModel::System), "name"),
            None,
            "a system-keyed tombstone must win over a later backend's value"
        );
    }

    #[test]
    fn detached_absent_state_can_replay_immutable_genesis_membership() {
        let model = test_model_id();
        let entity = Entity::create(EntityId::new(), model);
        entity.state.write().unwrap().pending_genesis_membership = None;

        let detached = entity
            .detached_at_state(None, Some(model))
            .expect("an immutable genesis event can rebuild an absent durable row after the resident marker is cleared");

        assert_eq!(detached.state.read().unwrap().pending_genesis_membership, Some(model));
    }
}
