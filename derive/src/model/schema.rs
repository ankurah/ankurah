...
pub fn validate_schema_attrs(model: &ModelDescription) -> syn::Result<()> {
    ...
    for field in model.active_fields() {
        ...
        if !property_names.insert(name.clone()) {
            return Err(syn::Error::new(ident.span(), format!("duplicate property name '{name}' after lowercasing")));
        }
...
pub fn schema_impl(model: &ModelDescription) -> syn::Result<TokenStream> {
    ...
    let system_tokens = match model.system() {
        Some(system) => {
            let variant = format_ident!("{}", <&str>::from(system));
    ...
    let model_resolved = match model.system() {
        Some(system) => {
            let variant = format_ident!("{}", <&str>::from(system));
            quote! { #base::schema::PerSystemOnceCell::Pinned(#base::proto::ModelId::System(#base::proto::SystemModel::#variant)) }
        }
        None => quote! { #base::schema::PerSystemOnceCell::per_epoch() },
    };
    ...
    for (field, active_type) in model.active_fields().iter().zip(active_types.iter()) {
        ...
        let field_resolved = match model.system() {
            Some(_) => {
                ...
                let variant = format_ident!("{}", <&str>::from(system));
                quote! { #base::schema::PerSystemOnceCell::Pinned(#base::proto::PropertyId::System(#base::proto::SystemProperty::#variant)) }
            }
            None => quote! { #base::schema::PerSystemOnceCell::per_epoch() },
        };
...
fn map_value_type(ty: &Type) -> FieldSchemaType<'_> {
