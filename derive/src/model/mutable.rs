...
pub fn mutable_impl(model: &crate::model::description::ModelDescription) -> TokenStream {
    ...
    // Generate WASM wrapper types and getter methods
    let (wasm_wrapper_impls, wasm_getter_impl) = if cfg!(feature = "wasm") {
        let wrapper_impls = model.backend_manager().generate_wasm_wrapper_impls(model);
        let getter_methods = model.backend_manager().generate_wasm_getter_methods(model);
    ...
    let expanded = quote! {
        ...
        pub struct #mutable_name {
            ...
            // #(
            //     #field_attributes
            //     pub #active_field_names: #active_field_types,
            // )*
        }
        ...
        // Generate WASM-only materialized wrapper types and getter methods
        #(#wasm_wrapper_impls)*
