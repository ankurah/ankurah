...
pub fn model_impl(model: &crate::model::description::ModelDescription) -> TokenStream {
    ...
    let active_field_resolutions = match crate::model::schema::active_field_resolution_tokens(model, quote! { entity }, quote! { model }) {
    ...
    let ref_wrapper_type = if cfg!(feature = "wasm") {
        if model.no_ffi() {
            quote! { type RefWrapper = ::ankurah::property::Ref<Self>; }
        } else {
            let ref_name = format_ident!("{}Ref", name);
            quote! { type RefWrapper = #ref_name; }
        }
    } else {
        quote! {}
    };
    ...
    quote! {
        impl ::ankurah::model::Model for #name {
            ...
            ) -> Result<(), ::ankurah::error::MutationError> {
