...
impl ModelDescription {
    ...
    ///
    /// - `Ref<T>` → `TRef` (via `<T as WasmModel>::RefWrapper`)
    ...
    pub fn wasm_getters(&self) -> Vec<TokenStream> {
        ...
        let r_method = quote! {
            ...
            pub fn __wasm_r(&self) -> <#model_name as ::ankurah::model::wasm::WasmModel>::RefWrapper {
                self.r().into()
            }
        };
        ...
        let field_getters = self.active_fields.iter().map(|field| {
            ...
            if is_ref {
                ...
                quote! {
                    ...
                    pub fn #wasm_method_name(&self) -> Result<<#inner_model as ::ankurah::model::wasm::WasmModel>::RefWrapper, JsValue> {
                        ::ankurah::core::model::wasm_prop(self.#field_name(), #field_name_str, #model_name_str)
                            .map(|r| <#inner_model as ::ankurah::model::wasm::WasmModel>::RefWrapper::from(r))
                    }
                }
                ...
                quote! {
                    #[doc(hidden)]
                    #[wasm_bindgen(getter, js_name = #field_name)]
                    pub fn #wasm_method_name(&self) -> Result<Option<<#inner_model as ::ankurah::model::wasm::WasmModel>::RefWrapper>, JsValue> {
                        ::ankurah::core::model::wasm_prop(self.#field_name(), #field_name_str, #model_name_str)
                            .map(|opt| opt.map(|r| <#inner_model as ::ankurah::model::wasm::WasmModel>::RefWrapper::from(r)))
                    }
                }
            } else {
