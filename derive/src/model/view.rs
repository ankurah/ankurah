...
pub fn view_impl(model: &crate::model::description::ModelDescription) -> TokenStream {
    ...
    #[cfg(feature = "wasm")]
    let ffi_attrs = if model.no_ffi() { plain_attrs() } else { super::wasm::view_attributes(&view_name, &mutable_name, &name) };

    #[cfg(all(feature = "uniffi", not(feature = "wasm")))]
    let ffi_attrs = if model.no_ffi() { plain_attrs() } else { super::uniffi::view_attributes() };

    #[cfg(not(any(feature = "wasm", feature = "uniffi")))]
    let ffi_attrs = plain_attrs();
    ...
    let expanded = quote! {
            ...
            impl<S: #base::ankql::ast::Stage> From<#view_name> for #base::ankql::ast::Expr<S> {
                fn from(view: #view_name) -> #base::ankql::ast::Expr<S> {
                    view.entity.id().into()
                }
            }
            ...
            impl #view_name {
                ...
                pub fn edit<'rec, 'trx: 'rec>(&self, trx: &'trx #base::transaction::Transaction) -> Result<#base::model::MutableBorrow<'rec, #mutable_name>, #base::error::RetrievalError> {
                    use #base::model::View;
