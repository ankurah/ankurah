...
use crate::tsify::{attrs::TsifyContainerAttrs, error_tracker::ErrorTracker};
...
impl<'a> Container<'a> {
    pub fn new(serde_container: SerdeContainer<'a>) -> Self {
        ...
        let name = attrs.ty_config.format_name(serde_container.attrs.name().serialize_name().to_string());

        let ident_str = attrs.ty_config.format_name(serde_container.ident.to_string());

        Self { errors, attrs, serde_container, ident_str, name }
    }

    pub fn from_derive_input(input: &'a syn::DeriveInput) -> syn::Result<Self> {
        let cx = serde_derive_internals::Ctxt::new();
        let serde_cont = SerdeContainer::from_ast(&cx, input, serde_derive_internals::Derive::Serialize);
    ...
    /// The `ident` of the type as written in the Rust code.
    pub fn ident(&self) -> &syn::Ident { &self.serde_container.ident }

    /// The `ident` of the type as written in the Rust code as a string.
    pub fn ident_str(&self) -> String { self.ident_str.clone() }

    #[inline]
    pub fn serde_attrs(&self) -> &attr::Container { &self.serde_container.attrs }

    /// Whether or not Serde has marked this type as `transparent`.
    pub fn transparent(&self) -> bool { self.serde_attrs().transparent() }
    ...
    pub fn check(self) -> syn::Result<()> { self.errors.check() }
}
