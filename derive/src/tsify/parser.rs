...
impl<'a> Parser<'a> {
    pub fn new(container: &'a Container<'a>) -> Self { Self { container } }
    ...
    fn create_relevant_type_params(&self, type_ref_names: HashSet<&String>) -> Vec<String> {
        self.container.generics().type_params().map(|p| p.ident.to_string()).filter(|t| type_ref_names.contains(t)).collect()
    }
    ...
    fn parse_fields(&self, style: Style, fields: &[Field]) -> ParsedFields {
        let style = match style {
            ...
            Style::Unit => return ParsedFields::Transparent(TsType::nullish(&self.container.attrs.ty_config)),
        };

        let fields = fields
            .iter()
            .filter(|field| !field.attrs.skip_serializing() && !field.attrs.skip_deserializing() && !is_phantom(field.ty))
            .collect::<Vec<_>>();
        ...
        match style {
            ...
            FieldsStyle::Unnamed => {
                let elems = fields.into_iter().map(|field| self.parse_field(field).0).collect();
    ...
    fn parse_field(&self, field: &Field) -> (TsType, Option<TsifyFieldAttrs>) {
        ...
        if let Some(t) = &ts_attrs.type_override {
            ...
            let type_params = self.create_relevant_type_params(type_ref_names);
            (TsType::Override { type_override: t.clone(), type_params }, Some(ts_attrs))
        } else {
    ...
    fn parse_named_fields(&self, fields: Vec<&Field>) -> (Vec<TsTypeElement>, Vec<TsType>) {
        let (flatten_fields, members): (Vec<_>, Vec<_>) = fields.into_iter().partition(|field| field.attrs.flatten());
        ...
        let members = members
            ...
            .map(|field| {
                ...
                let default_is_none = self.container.serde_attrs().default().is_none() && field.attrs.default().is_none();
                ...
                TsTypeElement { key, type_ann, optional: optional || !default_is_none, comments }
            })
            .collect();

        let flatten_fields = flatten_fields.into_iter().map(|field| self.parse_field(field).0).collect();
    ...
    fn parse_enum(&self, variants: &[Variant]) -> Decl {
        let members = variants
            ...
            .map(|variant| {
                ...
                if let Decl::TsTypeAlias(mut type_alias) = decl {
                    variant.attrs.name().serialize_name().clone_into(&mut type_alias.id);
        ...
        let type_ref_names = members.iter().flat_map(|type_alias| type_alias.type_ann.type_ref_names()).collect::<HashSet<_>>();
...
fn is_phantom(ty: &syn::Type) -> bool {
    if let syn::Type::Path(syn::TypePath { path, .. }) = ty {
        path.segments.last().is_some_and(|path| path.ident == "PhantomData")
    } else {
        false
    }
}
