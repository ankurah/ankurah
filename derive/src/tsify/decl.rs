...
use crate::tsify::comments::clean_comments;
use crate::tsify::{
...
impl TsTypeAliasDecl {
    pub fn to_string_with_indent(&self, indent: usize) -> String {
        ...
        out.split('\n').map(|line| format!("{}{}", indent_str, line)).collect::<Vec<_>>().join("\n")
    }
}
...
impl Display for TsInterfaceDecl {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if !self.extends.is_empty() {
            let extends = self.extends.iter().map(|ty| ty.to_string()).collect::<Vec<_>>().join(", ");
        ...
        if self.body.is_empty() {
            ...
            let members = self.body.iter().map(|elem| format!("\n{};", elem.to_string_with_indent(4))).collect::<Vec<_>>().join("");
            write!(f, " {{{members}\n}}")
        }
    }
}
...
impl TsEnumDecl {
    fn replace_type_params(ts_type: TsType, type_args: &mut Vec<String>) -> TsType {
        match ts_type {
            TsType::Ref { name, type_params } => TsType::Ref {
                ...
                type_params: type_params
                    ...
                    .map(|_| {
                        ...
                        TsType::Ref { name, type_params: Vec::new() }
                    })
                    .collect(),
            },
            TsType::Array(t) => TsType::Array(Box::new(TsEnumDecl::replace_type_params(t.deref().clone(), type_args))),
            TsType::Tuple(tv) => TsType::Tuple(tv.iter().map(|t| TsEnumDecl::replace_type_params(t.clone(), type_args)).collect()),
            TsType::Option(t, null) => TsType::Option(Box::new(TsEnumDecl::replace_type_params(t.deref().clone(), type_args)), null),
            TsType::Fn { params, type_ann } => TsType::Fn {
                params: params.iter().map(|t| TsEnumDecl::replace_type_params(t.clone(), type_args)).collect(),
            ...
            TsType::Intersection(tv) => {
                TsType::Intersection(tv.iter().map(|t| TsEnumDecl::replace_type_params(t.clone(), type_args)).collect())
            }
            TsType::Union(tv) => TsType::Union(tv.iter().map(|t| TsEnumDecl::replace_type_params(t.clone(), type_args)).collect()),
            _ => ts_type,
        }
    }
}
                    ...
                    type_refs
                        ...
                        .map(|(name, type_args)| {
                            ...
                            let ts_type = TsEnumDecl::replace_type_params(
                                TsType::Ref { name: name.clone(), type_params: type_args.clone() },
                                &mut type_refs,
                            );
            ...
            if self.members.is_empty() {
                ...
                let members = self
                    ...
                    .map(|elem| TsTypeAliasDecl {
                        ...
                        type_ann: elem.type_ann.clone().prefix_type_refs(&prefix, &self.type_params),
...
impl Decl {
    ...
    pub fn set_id(&mut self, id: String) {
        match self {
            Decl::TsTypeAlias(decl) => decl.id = id,
            Decl::TsInterface(decl) => decl.id = id,
            Decl::TsEnum(decl) => decl.id = id,
        }
    }
}
