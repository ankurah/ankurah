...
validate_published_crates() {
    ...
    if [[ -n "$missing" || -n "$unexpected" ]]; then
        release_error "$crates_file must list every publishable workspace crate exactly once"
        [[ -z "$missing" ]] || release_error "missing publishable crates: ${missing//$'\n'/, }"
        [[ -z "$unexpected" ]] || release_error "listed crates marked publish=false: ${unexpected//$'\n'/, }"
        return 1
    fi
