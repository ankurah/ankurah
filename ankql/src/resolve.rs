...
use crate::ast::{Expr, OrderByItem, OrderKey, PathExpr, Predicate, PropertyIdExt, PropertyPath, Selection};
...
fn cast_comparison_value(_model: &ModelId, expr: Expr, target: ValueType) -> Result<Expr, NameResolutionError> {
    Ok(match expr {
        // Best-effort: a literal that cannot take the property's registered
        // type stays as written, and the comparison evaluates leniently at
        // each consumer (which must re-cast at its own trust boundary
        // regardless). Erroring here would reject the whole query for one
        // non-conforming comparison -- but a policy filter deliberately
        // injects such comparisons (an EntityId-typed field held against a
        // distinguished string subject) with deny-the-row, never
        // error-the-query, semantics. [Deliberate divergence from the
        // reference implementation, which failed closed here; restoring
        // strictness once policy injection emits its own resolved, typed
        // comparisons is https://github.com/ankurah/ankurah/issues/472.]
        Expr::Literal(value) => match value.cast_to(target) {
            Ok(cast) => Expr::Literal(cast),
            Err(_) => Expr::Literal(value),
        },
        Expr::ExprList(values) => {
            Expr::ExprList(values.into_iter().map(|value| cast_comparison_value(_model, value, target)).collect::<Result<Vec<_>, _>>()?)
        }
        other => other,
    })
}
...
fn resolve_path_head<R: NameResolver + ?Sized>(
    ...
    if name == "id" {
        return Ok(PropertyPath::id());
    }
    let property = match model {
        ModelId::System(_) => SystemProperty::from_name(name).map(PropertyId::System),
        ModelId::EntityId(_) => {
            // Ask first: a layered resolver can answer some names
            // authoritatively (a compiled declaration's cells) even while
            // its fallback metadata is still arriving. Readiness qualifies
            // what a MISS means -- absent versus not-yet-known -- exactly as
            // the trait documents, so it is consulted only on `None`.
            match resolver.resolve_property(model, name)? {
                Some(property) => Some(property),
                None if !resolver.is_ready(model) => {
...
pub fn system_property(name: &str, subpath: Vec<String>) -> Option<PropertyPath> {
    if name == "id" {
        // The id pseudo-property has no subfields; any subpath is dropped
        // (the resolution pass rejects `id.<anything>` before reaching here).
        return Some(PropertyPath::id());
    }
...
mod tests {
    ...
    impl NameResolver for ColdResolver {
        ...
        fn resolve_property(&self, _model: &ModelId, _name: &str) -> Result<Option<PropertyId>, NameResolutionError> {
            // A cold resolver IS asked (a layered resolver may answer some
            // names while its metadata is still arriving); it answers
            // nothing, and unreadiness makes that miss report not-ready
            // rather than unknown.
            Ok(None)
        }
