...
backends**. A backend is the layer responsible for three things: holding values
keyed by durable `PropertyId`, applying mutations to those values, and deciding
what happens when two replicas change the same property concurrently. Human
property names are resolved by the view/accessor layer before the backend is
addressed.
...
Mutable accessors return transaction-bound property handles (defined in
`core/src/property/value/`). View getters read through the entity using the
descriptor's backend and convert the value with `Property::from_value`.

| Type | Backend | Purpose |
|------|---------|---------|
| `LWWMut<T>` | LWW | Scalar property; `T` implements `Property` (String, i64, Json, Ref, etc.) |
| `YrsStringMut<P>` | Yrs | Collaboratively-editable text with insert/delete/replace |
| `Json` | LWW (via `LWWMut<Json>`) | Structured JSON stored as a scalar |
| `Ref<T>` | LWW (via `LWWMut<Ref<T>>`) | Typed entity reference storing an `EntityId` |

Both `LWWMut<T>` and `YrsStringMut<P>` enforce write guards: calling a mutating
