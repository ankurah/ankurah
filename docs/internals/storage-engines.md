...
|--------|--------------------|--------------|--------------|--------------------|------------|
| **sled** (`storage/sled`) | Native, embedded KV; the default for servers and dev | Canonical state in a shared `entities` tree (the bincoded state struct carries the head and its per-tip `head_generations` annotation inside it); a per-collection `collection_{id}` tree holds the materialized property projection that indexes and scans use | Shared `events` tree keyed by event id; the mandatory event `generation` rides inside the bincoded struct | Shared `Planner` picks index vs. table scan; residual predicate/sort/limit via shared streams; sled ops run on `spawn_blocking` | On-disk sled db; `new()` under `~/.ankurah`, plus a temporary in-memory mode for tests |
| **postgres** (`storage/postgres`) | Native, production server backend | One table per collection (bare `{collection}`); columns added on demand as properties appear; each row carries `state_buffer`, `head`, `generations` (per-tip head generations, an array PARALLEL to `head`, length-checked on read), `attestations` | `{collection}_event` table keyed by `id`, with an `entity_id` column and a `NOT NULL` `generation` column (bigint, range-checked on read) | Predicate split into pushdown `WHERE` + Rust post-filter; `LIMIT` deferred past post-filter; DDL serialized with advisory locks | Full SQL server; connection pooled via `bb8` |
| **indexeddb-wasm** (`storage/indexeddb-wasm`) | Browser (WASM) client storage | Shared `entities` object store; entity records carry the per-tip head generations under a `__generations` key as `[generation, eventIdBase64]` pairs (checked fail-loud decode); per-collection index object stores for queries | Shared `events` object store with a `by_entity_id` index; each event record carries `__generation` (same checked fail-loud decode) | Shared `Planner` in `PlannerConfig::indexeddb()` mode (ascending-only indexes); IDB index cursors + residual filter/sort in Rust | Browser IndexedDB; `!Send`, wrapped in `SendWrapper` |
| **sqlite** (`storage/sqlite`) | Embedded single-file SQL; native incl. mobile (iOS/Android) | One table per collection (bare `{collection}`), columns added on demand; row carries `state_buffer`, `head`, `generations` (parallel to `head`, length-checked on read), `attestations` | `{collection}_event` table with a `NOT NULL` `generation` column (strictly typed read) and an explicit `entity_id` index for `dump_entity_events` | Same pushdown/post-filter split as Postgres, using SQLite JSON/JSONB operators for JSON paths | Single-file (or in-memory) SQLite via `rusqlite` "bundled"; pooled via `bb8` |
