...
`PreparedEntityWrite` contains the exact durable `expected_head` and the
proposed attested canonical state. The complete model set is already in that
state's explicit memberships; there is no separate association instruction or
trusted request context. `StorageWriteBatch` contains at most one write per
entity and is all-or-nothing.
...
3. replays the transaction's already-durable events and derives any membership
   changes from their explicit operations;
