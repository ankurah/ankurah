...
The one type check that necessarily stays on the write path is per-value: a
...
fail the mutation before an ill-typed value enters the generated accessor's
uncommitted set. Commit repeats the check for low-level/internal staging paths.
...
**Writes canonicalize before staging.** A generated mutable accessor already
knows its durable property id. Its `set` path resolves that property's
canonical catalog type and casts the value before placing it in the backend's
uncommitted set. A value the canonical type cannot represent therefore fails
at the writer without temporarily creating ill-typed entity state.

Commit repeats canonicalization before generating the event. This is a
defensive boundary for schema-blind or low-level code which stages directly
through a backend; it is not the primary generated-accessor path. Consequently
events, state buffers, storage engines, and downstream indexes still receive
one consistent type.
