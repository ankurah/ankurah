...
```text
  stage_event -----> compare/apply -----> StorageTransaction::commit
  (in-memory map)   (candidate state)    (events + CAS + projections)
```
...
3. **Commit** -- Persist the validated events and exact-head state replacement
   in one batch, refreshing associations and every affected model projection.

### The ordering invariant

> **Stage before candidate head update; persist events and canonical state together.**

The first half ensures BFS reachability. The second half ensures crash safety:

- An uncommitted batch exposes none of its new events, states, associations,
  or projections.
- A committed batch survives a lost acknowledgement; re-delivery is idempotent.
