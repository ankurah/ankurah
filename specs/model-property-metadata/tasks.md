...
handshake work was folded into #307 and the final unreleased 0.10.0 contract
is protocol version 2.

- [x] `protocol_version: u32` appended as the last field of `Presence`;
      compatibility isolated in one equality check. Version 2 represents the
      final PR #307 contract and refuses the incompatible version-1
      development shape that previously existed on `main`.
...
- [x] Wire shapes: LWW_DIFF_VERSION_2 and LWW_STATE_VERSION_2 (0xA2)
      carry one `BTreeMap<PropertyId, ...>`; round-trip tests.
- [x] Every property backend is addressed by `PropertyId` and holds no schema
      binding, display-name hint, or per-instance wire mode.
- [x] Compatibility is deliberately refused: 0xA1/name-keyed buffers do not
      decode and no display-name fallback survives.
- [x] Generated View/Mutable accessors resolve ordinary source names through
      the catalog before constructing an active field. LWW values are cast to
      the canonical property type before staging and checked again before
      event generation; Yrs roots use `PropertyId::to_string()`. System fields
      use the closed `PropertyId::System` arm.
- [x] Unknown-id v2 payloads apply and persist opaquely (catalog lag);
      unprojectable until a binding knows the id.
- [x] Compatibility tests: unknown and old state versions are refused; there
      is no legacy Name residue.
- [x] PROTOCOL_VERSION 2 covers LWW v2/0xA2, resolved
      predicate Identifiers and ORDER BY property identities, RegisterSchema,
      model-id wire envelopes, and model-independent event/state payloads.
