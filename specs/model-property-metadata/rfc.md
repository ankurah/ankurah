...
carrying this fact is `PropertyId::System { name }` (5.5); the data fact
itself is unchanged.
...
resolved `PropertyId { Id | EntityId(Ulid) | System { name } }` plus the JSON
...
resolves to `PropertyId::Id`; system/catalog properties resolve to
`PropertyId::System { name }`. The resolution-pass semantics below (bind at
...
the single durable identity `ankql::ast::PropertyId { Id | EntityId(Ulid) |
System { name } }` -- the same type the query layer already uses -- as the key
...
- A system/catalog field is `PropertyId::System { name }` -- still name-addressed
  by necessity (the bootstrap has no id to mint), now a first-class identity arm
  rather than a `Name` variant that doubles as a fallback.
