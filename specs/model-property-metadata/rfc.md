...
materializations. Engines durably track the set of models through which each
entity has been used and refresh every associated projection on canonical
writes. The source-facing `Model::collection()` vocabulary remains a
compatibility label for model registration and query context; it is not a
physical collection or an ownership claim. Multi-model entity use is therefore
supported by storage even though each typed handle still carries one explicit
model context.
...
- Yrs names each root as the JSON-encoded `PropertyId`. A root that does not
  decode as a `PropertyId` is refused at buffer ingest as malformed, under the
  same no-backcompat ruling (no legacy-name roots).
...
home sections: Yrs roots carry the uniform property identity (JSON-encoded
`PropertyId` since the PropertyKey retirement, PR #307), 5.5; required defaults and absent-as-NULL behavior are separated by consumer,
