...
epoch (version 2). Engines refuse
...
**Decision: dedicated built-in models, per #85 AC1.** `_ankurah_model` and
`_ankurah_property` are schema labels for closed `SystemModel` variants,
addressed at runtime as `ModelId::System(SystemModel)`. Metadata entities are ordinary
...
Model-is-not-Collection stance (the model entity records a source registration
label as data; today the label is the lowercased struct name,
...
shape the maintainer's storage trajectory requires. At the time of the
original proposal, an entity belonged to exactly one collection and the SQL
engines kept canonical state in that collection's table. The unified-storage
amendment in this revision realizes the planned direction (maintainer,
2026-07-05): a Model is a data contract an entity is ANTICIPATED to meet; one
entity can meet several contracts, and contracts overlap. Concretely on
...
SUPERSEDED by the unified-storage amendment ratified for PR #307
(`specs/storage/architecture.md`, 2026-07-23): canonical entities and events
are now model-independent engine records, while models define query
materializations. Membership is an explicit attested operation whose result is
stored in canonical `State.memberships`; engines privately persist/index that
set and refresh every member projection on canonical writes. The source-facing
model label is registration/query metadata, not a physical collection or an
ownership claim. `Entity` is model-independent; generated `View` and `Mutable`
values carry one explicit model projection. Storage is deliberately
many-to-many-shaped, while the current event protocol permits exactly one
genesis membership and defers add-to-existing/removal to #412.

**Bootstrap non-recursion.** The catalog models' own properties use the closed
`PropertyId::System(SystemProperty)` identities. The meta-schema (the
properties above and their types) is defined by this spec and frozen, so
catalog bootstrap needs no catalog lookup and introduces no recursive
registration. They still use the same backend encoding as other entities;
only their property-identity source is built in. Concretely (maintainer,
2026-07-05): catalog entities are SYSTEM MODELS, built and read
...
retirement (PR #307, 2026-07-15): with `PropertyKey` retired, the identity is
`PropertyId::System(SystemProperty)` (5.5), a closed enum whose variants have
canonical source names; the bootstrap data fact itself is unchanged.
...
transaction-path write ban replaces it. The `_ankurah_` prefix is reserved
and rejected for user-model schema labels at derive and registration time.
Because enforcement sits on the receiving durable node,
...
the closed `ModelId::System(SystemModel)` arm. `SystemModel` admits only the
known built-ins; no stringly name and no synthetic or reserved `EntityId`
stands in for one.
SUPERSEDED (review correction, #289, 2026-07-10): the earlier design warned
that a mutable catalog map could route an allocated id into a protected
collection. Runtime storage is now keyed directly by `ModelId`, so no catalog
name lookup can retarget an `EntityId` arm into a built-in. The transaction
guard validates/opens every model up front and rejects every `SystemModel`
before writing; registration's direct commit path bypasses that guard.
Descriptor ingest remains hardened: shipped definitions are ingested only
after recipient/connection checks, a wire model definition naming a reserved
schema label is rejected, and a label cannot be rebound to another model id.
...
- model: by source label (the compatibility descriptor field is named
  `collection`)
...
castable per the `Value::cast_to` relation (`core-types/src/value.rs`), and
...
carries the language-agnostic definitions: models (source label, display name),
...
properties name their target model by source label, resolved executor-side.
...
allocated or existing id). Catalog models are not writable any other
way (section 4 protection). A target-model reference whose label has never
been sighted allocates the model entity on the spot (label set, display name
arriving whenever that model properly registers), which
...
**SHAPE SUPERSEDED (column-layering + PropertyKey retirement, PR #307,
2026-07-15).** The `Identifier` draft below shipped as `PropertyPath`: a
resolved `PropertyId::{Id, EntityId(EntityId), System(SystemProperty)}` plus the JSON
`subpath`, with NO resolved-at `name` field. Display names live in the
catalog; physical columns are each engine's private concern, derived from its
durable identity-to-column map at emit time, and engines refuse unresolved
selections outright (`Selection::check`, 5.5). The `id` pseudo-property
resolves to `PropertyId::Id`; system/catalog properties resolve to a closed
`PropertyId::System(SystemProperty)` variant. The resolution-pass semantics below (bind at
the queried collection, fail closed with `UnknownProperty`) shipped unchanged.
...
programmatic construction, then resolve through AnkQL's `NameResolver`; this
replaced the former core `TypeResolver` call sites). On nodes with no
...
**AMENDED (PropertyKey retirement, PR #307, 2026-07-15): rule 2's id-then-name
fallback is RETRACTED.** A read is by durable identity alone (`read_by_id`
keyed on `ankql::ast::PropertyId`); an absent entry is absent, with NO fallback
to a name-keyed slot (legacy `Name` residue is gone -- see 5.5). Rules 1 and 3
stand unchanged.
...
2. ~~Generic backend dispatch reads the resolved Id entry first. A present Id
   ...
   may fall back to legacy Name residue.~~ (RETRACTED, see the amendment above:
   the Id entry is authoritative and there is no name fallback.)
...
**SUPERSEDED (PropertyKey retirement, PR #307, 2026-07-15). `PropertyKey` is
RETIRED.** The two-arm `PropertyKey { Id | Name }` described below is replaced by
the single durable identity `PropertyId::{Id, EntityId(EntityId),
System(SystemProperty)}` -- the same type the query layer already uses -- as the key
for property backends and value accessors too. What that retracts here and in
5.4:
- Resolution (name -> `PropertyId`) happens ONCE, at editable-view construction
  (`from_entity`), erroring `UnknownProperty` when the catalog cannot bind the
  name. There is NO transient `Name` staging and NO commit-time `Name`->`Id`
  rekey; a write to an unbound field fails at `create`/`set`, not at `commit`.
- Property backends are dumb, catalog-free `PropertyId`->value stores. A read is
  by identity via `read_by_id`, with NO id-then-name fallback (this retracts the
  5.4 read-dispatch name fallback).
- The wire/state value map re-serializes as `BTreeMap<PropertyId, ..>` (still one
  tagged 0xA2/v2 container). The legacy v1/0xA1 decode is DELETED -- no backward
  compatibility this release; a dev DB is reset, not migrated (a migration
  function is a later concern).
- A system/catalog field is `PropertyId::System(SystemProperty)` -- a closed
  bootstrap identity (there is no catalog id to mint), not a stringly name or a
  `Name` fallback.
The #289 PropertyKey amendment text below is retained as HISTORICAL design record.

The PropertyKey amendment (#289, 2026-07-07) was the prior contract. It
...
system and catalog models have no model-definition entities, so they use the
closed `SystemModel` enum. Serde encodes the arms honestly. No EntityId byte
pattern is reserved, classified, or reinterpreted as a system model. EventId
already excluded the model address from identity, so event ids are unchanged.
Source labels survive in query/API and registration payloads, but the system
arm admits only the built-in enum variants; arbitrary strings cannot bypass
catalog allocation. `EntityId`
...
retries before rejecting (`prepare_model_for_ingress`) -- a restarted node
...
fire. This is an incompatible development-era wire change. The final PR #307
contract advertises protocol version 2 and refuses the intermediate
version-1 contract that appeared on `main`; neither development contract has
been released. Existing development peers must restart together and
development stores containing old sled event blobs must reset; there is
deliberately no compatibility decoder for the old envelope.
...
hold no schema binding or wire mode; every entry is addressed by the resolved
`PropertyId` supplied by the catalog-aware caller (the PropertyKey retirement,
PR #307, 2026-07-15, replaced the interim two-arm `PropertyKey { Id | Name }`
recorded in the 5.5 banner above).

- LWW diff version 2 and state version 0xA2 each encode one
  `BTreeMap<PropertyId, ...>`. The legacy v1/0xA1 decode is deleted (no
  backward compatibility; dev databases reset), and there is no Name residue:
  resolution happens once at editable-view construction, so entries are
  identity-keyed from the first write. A present entry, including a
  tombstone, is authoritative; there is no name fallback.
- Yrs names each root as `PropertyId::to_string()`: a registered property is
  its 22-character URL-safe base64 entity id and a system property is its
  closed enum name. A root that is neither form is refused at buffer ingest
  as malformed, under the same no-backcompat ruling (no legacy-name roots).
- Query ASTs carry resolved `PropertyPath` nodes, and each resolved ORDER BY
  key carries the same stable property identity. The identity keeps a
  relayed/rebuilt sort stable across a catalog rename; no display path
  travels beside it (the rev-4 draft kept the path as a SQL/physical-column
  ...
  riding the same unreleased 0.10.0 protocol-2 epoch.

Materialized storage is a separate concern. Every engine maintains a durable
property-id to physical-field map, seeded from the injected catalog resolver
during first projection preparation and consulted on reads. PostgreSQL uses
`_ankurah_postgres_column_map`, SQLite uses
`_ankurah_sqlite_column_map`, sled uses
`_ankurah_sled_property_map`, and IndexedDB uses the `property_columns` object
store. The first claimant receives the sanitized display name; a collision
appends a widening suffix from the property id. Renames do not change identity,
and physical assignments remain sticky until explicit
rename/rematerialization work changes them. A selection is
checked at the engine boundary (`Selection::check` refuses raw paths and
unpopulated placeholders) and identities translate to physical columns at
emit through the durable map, with no name fallback; ORDER BY retains the
property's canonical value_type even when its physical column is a sticky
pre-rename assignment. Empty scans resolve the model lazily, so an absent
row reports `EntityNotFound` rather than a cold-catalog error. The durable
model-to-table map remains deferred with #304.

These changes ship together behind the unreleased 0.10.0 protocol version 2. Presence negotiation refuses
...
SQL columns do not yet follow a rename: assignments stay sticky under the
durable map (5.5), and the catalog-driven RENAME COLUMN DDL remains Phase C
(9). Display-name updates are ordinary provenance-ordered follow-ups
...
- No source-label renames (the model entity records the label as data; acting
  on it is future schema-evolution work).
...
- **specs/unified-refs-edges-json/phase-3-schema.md (in-repo plan, unbuilt):
  ...
  former TypeResolver TODO (then at core/src/type_resolver.rs:24-26) is
  discharged by AnkQL name resolution. The phase-3 plan should be revised against this RFC
...
- **Phase A (one protocol epoch; durable nodes upgrade first):** the three
  ...
  and id-named roots for registered properties (5.5). **As DELIVERED, the
  identity keying diverges (PropertyKey retirement + column-layering,
  PR #307, 2026-07-15):** `PropertyKey`, the legacy Name fallback reads, and
  id-then-legacy-name dispatch were retired before merge. Backend maps, wire
  payloads, and Yrs roots key by `ankql::ast::PropertyId`; engines refuse
  unresolved selections (`Selection::check`) and translate identity to
  columns through their durable maps with no name fallback (5.5).
...
(Resolved during independent review and later amendments, recorded in their
home sections: Yrs roots carry `PropertyId::to_string()` (base64 entity id or
closed system-property name) since the PropertyKey retirement, PR #307, 5.5;
required defaults and absent-as-NULL behavior are separated by consumer,
...
- Code (all at 05593d0d): core/src/system.rs (catalog precedent);
  ...
  versioning, field broadcasts); core/src/property/mod.rs (resolved
  PropertyId access, Option projection);
  derive/src/model/{model,description,backend_registry}.rs
