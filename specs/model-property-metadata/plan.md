...
7. **Field broadcasts key by the same PropertyKey as the value/root.**
   ...
   (PR #307, 2026-07-15): broadcasts key by `PropertyId`; Yrs roots are the
   JSON-encoded `PropertyId` and legacy name roots are refused (RFC 5.5).
...
29. **The honest model-id wire envelope and descriptor shipping** (model-id
    ...
    SubscriptionUpdateItem carry
    `ModelId::{EntityId(EntityId), System(SystemModel)}` in place of the former
    string collection address; EventId already excluded collection
    ...
    (collection is the data there, #305). Deserialization admits only known
    `SystemModel` variants; ingress resolves `EntityId` through the catalog;
