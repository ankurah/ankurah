...
Phase 0 (#294 protocol version)            <- gates every wire change
  |
Phase A, one released protocol epoch (proto v1 -> v2), build order (rev 4):
...
(A1, A3, A8 wire shapes; A2 was deleted by rev 4, above) lands first as
red-to-green commits, then the
...
- `Presence` carries `protocol_version: u32` as its last field and the
  unreleased 0.10.0 epoch exports `PROTOCOL_VERSION = 2`. Pre-versioning binaries are
...
### A4. Catalog models, protection constants, prefix reservation

- The built-ins are closed `SystemModel` variants addressed as
  `ModelId::System(SystemModel)`. Their `_ankurah_*` collection strings are
  schema/registration labels, not runtime storage identities.
- The dead protected-collection constant becomes the central
  ...
  receiver-side commit guard (A6). Derive and registration reject the
  `_ankurah_` prefix for user schema labels; `CollectionSet` receives only
  already-typed `ModelId` values.
...
**RETRACTED by the PropertyKey retirement (PR #307, 2026-07-15): `PropertyKey`
is retired to `ankql::ast::PropertyId`; the value map serializes as
`BTreeMap<PropertyId, ..>` (0xA2/v2 only, no legacy decode). See RFC 5.5 and
decision 27. The bullets below are historical.**
...
- A resolution pass Selection -> resolved Selection binds steps[0] against
  ...
  AnkQL's `NameResolver::property_value_type` validates whole-property
  literals; JSON subpaths use JSON semantics because the catalog does not type
...
PR #307 ships the whole unreleased 0.10.0 epoch as PROTOCOL_VERSION = 2: LWW v2/0xA2,
...
The intermediate version-1 contract on `main` is incompatible with the final
PR #307 shapes, so version 2 refuses those development clients explicitly;
neither version has been published as a release.
...
with IndexedDB identity-bound (a `property_columns` store keyed by the
serialized `PropertyId`), not name-bound as the sketch above had it.
...
3. **PROTOCOL_VERSION is u32; the final unreleased 0.10.0 contract is
   version 2.** Pre-versioning binaries are implicitly version 0. Version 1
   existed on `main` during development; PR #307's final wire/state shapes are
   incompatible with it, so they advertise a distinct version even though
   neither development contract was released.
4. **One tagged PropertyKey map** in LWW diff v2/state 0xA2. ~~`Id` and
   ...
   `PropertyId` and legacy Name residue is gone (RFC 5.5).
5. **Uniform PropertyKey::{Id, Name}** across every backend, with no
   per-instance keying mode, schema binding, or wire mode. RETIRED
   (PR #307, 2026-07-15): the uniformity survives as
   `PropertyId::{Id, EntityId(EntityId), System(SystemProperty)}` (RFC 5.5).
...
7. **Field broadcasts key by the same PropertyKey as the value/root.**
   ...
   its Name root until an Id root exists.~~ RETIRED with PropertyKey
   (PR #307, 2026-07-15): broadcasts key by `PropertyId`; Yrs roots are
   `PropertyId::to_string()` (base64 entity id or closed system-property
   name), and legacy display-name roots are refused (RFC 5.5).
...
9. **Phase 0 folded into PR #307.** The standalone PR closed unmerged; the
   handshake and metadata wire changes ship as the unreleased v2 epoch.
...
16. **Commit-time registration closes the edit-only gap**: the sync
    ...
    EXTENDED (PropertyKey amendment, #289, 2026-07-07). **RETRACTED by the
    PropertyKey retirement (PR #307, 2026-07-15): there is no transient
    `Name`-key rekey at commit.** Resolution moved EARLIER, to editable-view
    construction (`from_entity`), which resolves each field to its `PropertyId`
    or errors `UnknownProperty` before any write; `commit_local_trx` keeps only
    the value canonicalization pass, not a key rekey. An explicit-id accessor
    still stages its literal id directly. See decision 27. The rest of this
    paragraph is historical: ~~commit_local_trx also RESOLVES each entity's
    transient uncommitted `Name` keys to their `Id` at this same point.~~
...
27. **The PropertyKey backend contract** (the PropertyKey amendment, #289,
    2026-07-07). **RETRACTED by the PropertyKey retirement, PR #307,
    2026-07-15 (see RFC 5.5).** `PropertyKey` is retired to
    `ankql::ast::PropertyId` as the one backend/accessor key; name->id
    resolution moves EARLIER, to editable-view construction (`from_entity`,
    erroring `UnknownProperty` on an unbound field), so the `commit_local_trx`
    transient-`Name`->`Id` rekey below is DELETED (commit keeps only
    canonicalization), backends never touch the catalog, reads have no
    id-then-name fallback, and the legacy 0xA1/v1 decode is removed. A system
    field is `PropertyId::System(SystemProperty)`. The text below is HISTORICAL.

    Every property backend keys by a uniform
...
28. **Engine-owned durable property-id -> column maps, landed with Phase A**
    ...
    mappings; neither Node, System, nor CatalogManager bears it): SQL and
    IndexedDB engines persist a property-id -> physical-name map, seeded from an injected
    resolver (`StorageEngine::set_catalog_resolver`, wired at `Node::build`)
    resolver at set_state and consulted on every read. PostgreSQL and SQLite
    use `_ankurah_postgres_column_map` and `_ankurah_sqlite_column_map`, with
    a uniqueness constraint per materialization and winner read-back so
    concurrent claimants converge. Sled instead persists a database-wide bidirectional
    `PropertyId` <-> numeric-slot map.
    ...
    RETRACTED by the column-layering (PR #307, 2026-07-15):
    selection_to_column_space and every name fallback are deleted. Each
    engine's fetch_states REFUSES an unresolved selection at the seam
    (Selection::check is its first statement) in addition to the
    resolution-pass rejection; identities translate to columns at emit
    through the engine's durable map alone; and system-collection queries
    address `PropertyId::System(SystemProperty)` identities, resolved like any
    other reference.
    Table creation and column creation timing are UNCHANGED (tables on
    first need, columns on first write). Buckets retain the input `ModelId`
    directly; hydration never reverse-resolves a model through the catalog, so
    an empty scan remains empty and an absent-entity `get_state` remains
    `EntityNotFound`. Rename DDL stays Phase C. Model materialization
    registration is now engine-owned too: Postgres,
    SQLite, and IndexedDB durably map `ModelId` to private names, while sled
    uses reversible identity-derived tree names. Buckets are constructed only
...
29. **The honest model-id wire envelope and descriptor shipping** (model-id
    ...
    `PROTOCOL_VERSION = 2`). Model-scoped query/delta/update envelopes carry
    `ModelId::{EntityId(EntityId), System(SystemModel)}` in place of the former
    string collection address. Canonical `Event` and `EntityState` are
    model-independent; genesis membership is an attested operation included in
    EventId and accumulated into `State.memberships`. Registered models use the
    real id of their catalog definition. System and catalog models have no such
    entity and use a closed `SystemModel` variant. The tagged wire arms are
    distinct and no EntityId range has special meaning. Registration labels
    otherwise survive only in query/API and registration payloads.
    Deserialization admits only known `SystemModel` variants; ingress resolves
    `EntityId` through the catalog;
    ...
    INGRESS side via `prepare_model_for_ingress`: a durable node whose startup
    ...
    half stays with #312. SUPERSEDED by the unified-storage amendment
    implemented in this PR: canonical entities/events are global, genesis
    carries an attested membership add, `State.memberships` is canonical, and
    engines derive private association indexes and model materializations from
    that state. Add-to-existing and removal remain deferred to #412.
30. **Canonical value_type; registration is a compatibility check, never a
    ...
    admits the same castable drift. The cast discipline: a typed accessor
    resolves its `PropertyId` and canonicalizes before placing a value in the
    uncommitted backend set; commit repeats that check defensively for
    low-level/internal staging paths. A value the canonical type cannot
    represent fails that writer. The bound getter casts
    canonical->compiled (per-value failure =
    `PropertyError::NonCastable`); the resolution pass casts comparison literals opposite a
    resolved whole-property reference (subpaths use AnkQL's JSON semantics; a
    literal the canonical type cannot represent fails the query loud at
    origin); every execution consumer re-casts at its own trust boundary.
    PostgreSQL follows its materialized column type; SQLite prefers catalog
    type and falls back to affinity; sled/IndexedDB use catalog type; the
    reactor casts both watcher thresholds and changed values, waking all
    relevant watchers for exact evaluation if a changed value cannot cast.
    Motivation: exactly one stored type per
...
31. **Read-time gates removed; backends addressed only through the
    ...
    `property::read_by_id`; every by-name backend touch and downcast
