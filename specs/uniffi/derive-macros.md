...
The same `#[derive(Model)]` attribute generates both WASM and UniFFI bindings, controlled by feature flags. The features are mutually exclusive:

```rust
#[cfg(all(feature = "wasm", feature = "uniffi"))]
compile_error!("Only one bindings feature can be enabled at a time.");
```
