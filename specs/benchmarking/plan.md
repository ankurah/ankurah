...
- Establish sprout as the first reusable benchmark suite that characterizes create/fetch/get/live-query workloads across tunable topologies.
...
3. **Workload catalog**: sprout exposes a documented set of scenarios (seed dataset, mixed fetches, limit/gap queries, subscription churn, concurrent writers) that can be executed against any supplied configuration.
...
1. **Define benchmark interfaces**
   - Within sprout (not a shared crate), create the traits/structs for `DurableNodeFactory`, `EphemeralNodeFactory`, `ConnectorFactory`, and the topology descriptor the suite requires.
...
4. **Execution surface**
   - Integrate sprout with `cargo bench`, exposing each benchmark case as a named bench function that orchestrates its scenario and prints metrics.
...
- **Crate layout**: sprout ships as a dedicated library crate under `lib/sprout`; benchmark entry points live in `benches/` so `cargo bench -p sprout` can execute discrete cases.
...
- **Phase 1 (sprout implementation)**: build the harness, parameter plumbing, scenarios, and reporting for local/manual usage.
