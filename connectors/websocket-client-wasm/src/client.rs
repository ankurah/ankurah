...
impl WebsocketClient {
    ...
    pub fn js_connection_state(&self) -> ConnectionStateEnumSignal {
        // TODO: This is a temporary workaround - should use a mapped signal
        // For now, create a Mut signal that holds the converted value
        let converted_signal = Mut::new(self.inner.state.value().into());
        ConnectionStateEnumSignal { sig: converted_signal.read(), handle: Box::new(()) }
    }
}
