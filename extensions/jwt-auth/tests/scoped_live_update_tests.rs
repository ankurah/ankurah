...
/// The same fail-loud contract surfaces through a live query. The synchronous
/// constructor cannot perform first-use registration, so it rejects a schema
/// that has not already been admitted and directs callers to `query_wait` or
...
async fn unregistered_collection_live_query_fails_loud() -> anyhow::Result<()> {
    ...
    let error = match member_ctx.query::<ScopeItemView>("label = 'x'") {
        Ok(_) => panic!("synchronous live query unexpectedly admitted an unregistered collection"),
        Err(error) => error,
    };
    let message = error.to_string();
    assert!(
        message.contains("has not been admitted") && message.contains("ScopeItem"),
        "actionable error naming the model and admission remedy, got: {message}"
    );
