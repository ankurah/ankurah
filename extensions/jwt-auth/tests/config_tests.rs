...
#[test]
fn test_wildcard_satisfies_configured_privileges() {
    let config = load_blog_config();
    ...
    assert!(!config.can_scan_collection(&admin, "anything_else"));
    assert!(!config.can_write_collection(&admin, "anything_else"));
}

#[test]
fn test_wildcard_requires_an_explicit_operation_grant() {
    let config = load_minimal_config();
    let admin = [String::from("Admin")];
    assert!(config.roles_have_privilege(&admin, "any_named_privilege"));
    assert!(!config.can_access_collection(&admin, "anything"));
    ...
    let config: PolicyConfig = serde_json::from_str(r#"{
        ...
        "collections": {
            "read_only": { "read": "read" },
            "retrieve_only": { "retrieve": "retrieve" },
    ...
    for (label, retrieve, scan, write) in [
        ...
        ("write_only", true, true, true),
        ("disabled", false, false, false),
    ] {
        assert_eq!(config.can_access_collection(&admin, label), retrieve, "{label}");
        assert_eq!(config.can_scan_collection(&admin, label), scan, "{label}");
        assert_eq!(config.can_write_collection(&admin, label), write, "{label}");
    }
}
