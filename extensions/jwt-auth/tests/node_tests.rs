...
async fn test_update_scope_requires_before_and_after_state() -> anyhow::Result<()> {
    ...
    let root = node.context(JwtContext::system())?;
    let scoped_record_model = root.register::<ScopedRecord>().await?;
    ...
    let retag_event =
        Event { entity_id: denied_record.id(), operations: OperationSet::default(), parent: Clock::new(denied_record.head().to_vec()) };

    let result = agent.check_event(&node, &context, &scoped_record_model, &denied_record, &allowed_record, &retag_event);
