...
impl JwtAgent {
    ...
    fn policy_catalog_for_model(
        &self,
        model: &ankurah_core::ModelId,
    ) -> Result<(Arc<dyn ankurah_core::schema::CatalogResolver>, String), AccessDenied> {
        ...
        resolvers
            ...
            .find_map(|resolver| resolver.model_name(model).ok().map(|name| (resolver, name.to_ascii_lowercase())))
    ...
    fn policy_model_name(&self, model: &ankurah_core::ModelId) -> Result<String, AccessDenied> {
        self.policy_catalog_for_model(model).map(|(_, name)| name)
    }
}
...
impl PolicyAgent for JwtAgent {
    ...
    fn check_event<SE: StorageEngine>(
        ...
        if ankurah_core::schema::is_catalog_collection(model) {
        ...
        let (resolver, model_name) = self.policy_catalog_for_model(model)?;
        ...
        if !entity_before.head().is_empty() {
            enforce_write_scope(&state.config, cdata, entity_before, model, &model_name, resolver.as_ref())?;
        }
        enforce_write_scope(&state.config, cdata, entity_after, model, &model_name, resolver.as_ref())?;
        Ok(None)
    }
    ...
    fn check_read<C>(
        ...
        state: &proto::State,
        _resolver: Option<std::sync::Weak<dyn ankurah_core::schema::CatalogResolver>>,
    ) -> Result<(), AccessDenied>
    where
        C: Iterable<Self::ContextData>,
    {
        ...
        let (resolver, model_name) = self.policy_catalog_for_model(collection)?;
        let guard = self.state.read().unwrap_or_else(|e| e.into_inner());
        if guard.config.scope_rules_for_collection(&model_name).is_empty() {
            return Ok(());
        }

        // Policy filters are authored with names, but canonical state is
        // id-keyed. Bind the inspection view for type-aware reads and resolve
        // every filter to durable property identities before evaluation.
        let resolver_weak = Arc::downgrade(&resolver);
        let entity = TemporaryEntity::new_bound(*id, collection.clone(), state, Some(resolver_weak))
            .map_err(|_| AccessDenied::ByPolicy("Read scope entity state could not be evaluated"))?;
        enforce_read_scope(&guard.config, data, &entity, collection, &model_name, resolver.as_ref())
    }
    ...
    fn check_write(
        ...
        let state = self.state.read().unwrap_or_else(|e| e.into_inner());
        if !state.config.can_write_collection(cdata.roles(), &model_name) {
            Err(AccessDenied::CollectionDenied(*model))
        } else {
            enforce_write_scope(&state.config, cdata, entity, model, &model_name, resolver.as_ref())
        }
    }
...
fn resolve_scope_predicate(
    predicate: &Predicate,
    model: &ankurah_core::ModelId,
    resolver: &dyn ankurah_core::schema::CatalogResolver,
) -> Result<Predicate, AccessDenied> {
    predicate.resolve_names(model, resolver).map_err(|_| AccessDenied::ByPolicy("Policy scope predicate could not be resolved"))
}
...
fn enforce_write_scope(
    ...
    for predicate in scoped_predicates(config, model_name, claims, ScopeAccess::Write)? {
        let predicate = resolve_scope_predicate(&predicate, model, resolver)?;
