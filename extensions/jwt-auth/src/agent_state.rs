    ...
    *policy_livequery.lock().unwrap_or_else(|e| e.into_inner()) = Some(lq);
    ...
    ankurah_core::task::spawn(async move {
        ...
        if let Some(node) = weak_node.upgrade() {
            ...
            // Rule predicates resolve raw names through the catalog, so wait
            // for its first durable answer before binding policy.
            if let Err(e) = node.catalog.wait_synced().await {
                tracing::warn!("ephemeral policy sync proceeding without a synced catalog: {e}");
            }
            if let Err(e) = node.catalog.ensure_schema_for_use(&node, &JwtContext::NoUser, crate::JwtPolicy::descriptor()).await {
