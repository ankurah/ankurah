...
impl ReactObserver {
    ...
    pub fn get_snapshot(&self) -> js_sys::BigInt {
        js_sys::BigInt::from(self.0.version.load(Ordering::Relaxed) as u64)
    }
    ...
    pub fn js_signal_count(&self) -> usize {
        self.0.entries.read().expect("entries lock is poisoned")
            .values()
