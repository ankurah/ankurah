...
//!
//! An entity id IS the id of its genesis event, so the first id demand freezes
//! the mutations so far and derives the id from them. These tests pin that the id a caller
...
async fn durable_node() -> Result<TestNode> {
    let node = Node::new_durable(Arc::new(SledStorageEngine::new_test()?), PermissiveAgent::new());
    node.system.create().await?;
    Ok(node)
}

async fn entity_events(node: &TestNode, id: proto::EntityId) -> Result<Vec<proto::Event>> {
    Ok(node.storage.dump_entity_events(id).await?.into_iter().map(|e| e.payload).collect())
}
...
async fn created_id_is_the_committed_genesis_id() -> Result<()> {
    let node = durable_node().await?;
    let ctx = node.context_async(c).await?;

    let trx = ctx.begin();
    let album = trx.create(&Album { name: "Kind of Blue".to_owned(), year: "1959".to_owned() }).await?;
    // Demand identity before commit, freezing the genesis now.
    let id = album.id();
    trx.commit().await?;

    let events = entity_events(&node, id).await?;
...
async fn identical_payloads_mint_distinct_entities() -> Result<()> {
    ...
    let first_genesis = entity_events(&node, first).await?.into_iter().find(|e| e.is_entity_create()).expect("first genesis");
    let second_genesis = entity_events(&node, second).await?.into_iter().find(|e| e.is_entity_create()).expect("second genesis");
...
/// Editing after demanding identity produces two events, not one
...
/// Typed creation includes its model membership in the frozen genesis,
/// so that membership participates in deriving the entity id.
#[tokio::test]
async fn typed_creation_includes_its_model_membership() -> Result<()> {
    let node = durable_node().await?;
    let ctx = node.context_async(c).await?;
    ...
    let genesis = entity_events(&node, id).await?.into_iter().find(|e| e.is_entity_create()).expect("genesis");
...
async fn a_relayed_genesis_is_the_one_create_minted() -> Result<()> {
    ...
    let stored_genesis = entity_events(&server, id).await?.into_iter().find(|e| e.is_entity_create()).expect("the peer stored it");
