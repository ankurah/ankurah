...
async fn persisted_root_restores_the_system_before_ephemeral_catalog_readiness() -> anyhow::Result<()> {
    tokio::time::timeout(Duration::from_secs(30), async {
        ...
        for durable in [true, false] {
            ...
            assert_eq!(node.context(DEFAULT_CONTEXT).is_ok(), durable);
        }
        Ok(())
    })
    .await?
...
async fn different_system_halts_by_default_without_wiping_storage() -> anyhow::Result<()> {
    tokio::time::timeout(Duration::from_secs(30), async {
        ...
        let materializations = engine.list_materializations().await?;
        ...
        assert_eq!(engine.list_materializations().await?, materializations);
        assert_eq!(engine.get_state(id).await?, state);
...
async fn enabled_replacement_wipes_storage_for_the_next_node() -> anyhow::Result<()> {
    tokio::time::timeout(Duration::from_secs(30), async {
        ...
        assert!(engine.get_state(id).await.is_ok());
        assert!(engine.get_state(root.payload.entity_id).await.is_ok());
        ...
        assert!(engine.list_materializations().await?.is_empty());
        ...
        assert!(engine.get_state(id).await.is_err());
        assert!(engine.dump_entity_events(id).await?.is_empty());
