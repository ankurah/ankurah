...
fn foreign_id_state_buffer(foreign_id: EntityId, value: Value, event_id: proto::EventId) -> Vec<u8> {
    ...
    backend.set(PropertyId::EntityId(foreign_id.to_ulid()), Some(value));
...
async fn foreign_id_data_is_not_this_field() -> anyhow::Result<()> {
    ...
    assert_eq!(read_by_id(&backend, &PropertyId::EntityId(a.to_ulid())), None, "another id's data must not substitute for this field");
    // Reading as B finds B's value, exactly as stored.
    assert_eq!(read_by_id(&backend, &PropertyId::EntityId(b.to_ulid())), Some(Value::I64(30)));
...
fn membership_genesis(model: &EntityId, property: &EntityId) -> proto::Event {
    let backend = LWWBackend::new();
    backend.set(PropertyId::System { name: "model".to_string() }, Some(Value::EntityId(*model)));
