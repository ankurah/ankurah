...
//!     resolved read (`read_by_id`) consults exactly its own id slot, with no
//!     name fallback, and never substitutes another id's value. Type admission is
...
use ankurah::core::property::{read_by_id, FromEntity, PropertyAddress};
...
/// store, so we key the value directly under `PropertyId::EntityId(foreign_id)`
/// (no binding, no display-name hint -- both were withdrawn by the PropertyKey
...
fn foreign_id_state_buffer(foreign_id: EntityId, value: Value, event_id: proto::EventId) -> Vec<u8> {
    ...
    backend.set(PropertyId::EntityId(foreign_id), Some(value));
...
async fn foreign_id_data_is_not_this_field() -> anyhow::Result<()> {
    ...
    assert_eq!(read_by_id(&backend, &PropertyId::EntityId(a)), None, "another id's data must not substitute for this field");
    // Reading as B finds B's value, exactly as stored.
    assert_eq!(read_by_id(&backend, &PropertyId::EntityId(b)), Some(Value::I64(30)));
...
async fn required_absent_i64_reads_type_default_zero() -> anyhow::Result<()> {
    // A bare entity with an empty LWW backend and explicit durable property
    // identities. Reading an absent REQUIRED i64 projection reads the type
    // default 0 (rule 3), while a REQUIRED String reads "" and a REQUIRED bool
    // reads false.
    let entity = Entity::create(EntityId::new(), ModelId::EntityId(EntityId::new()));
    // Touch the LWW backend so it exists on the entity.
    let _ = entity.get_backend::<LWWBackend>().unwrap();

    let count: LWW<i64> = LWW::from_entity(PropertyAddress::new("count", Some(EntityId::new())), &entity);
    assert_eq!(count.get()?, 0, "absent required i64 reads the type default 0");

    let title: LWW<String> = LWW::from_entity(PropertyAddress::new("title", Some(EntityId::new())), &entity);
    assert_eq!(title.get()?, "", "absent required string reads the empty string (the #175 case)");

    let flag: LWW<bool> = LWW::from_entity(PropertyAddress::new("flag", Some(EntityId::new())), &entity);
...
async fn optional_absent_reads_none_and_is_null_matches() -> anyhow::Result<()> {
    ...
    // None, so the inner i64 default never fires. Give the bare entity an
    // explicit durable property identity; ordinary names require a catalog.
    let entity = Entity::create(EntityId::new(), ModelId::EntityId(EntityId::new()));
    let _ = entity.get_backend::<LWWBackend>().unwrap();
    let opt: LWW<Option<i64>> = LWW::from_entity(PropertyAddress::new("count", Some(EntityId::new())), &entity);
    ...
    let record_model = ctx.model_id::<Record>().await?;

    // `note IS NULL` matches the entity that lacks note (absent -> NULL).
    let is_null = ankql::parser::parse_selection("note IS NULL")?.resolve_names(&record_model, node.catalog.resolver())?;
    ...
    let eq = ankql::parser::parse_selection("note = 'x'")?.resolve_names(&record_model, node.catalog.resolver())?;
...
/// `note` so a later AnkQL name-resolution pass binds it.
async fn register_optional_note(node: &Node<SledStorageEngine, PermissiveAgent>) -> anyhow::Result<()> {
    ...
    for _ in 0..100 {
        if let Some(model) = node.catalog.model_id_for("record") {
            if node.catalog.resolve(&model, "note").is_some() {
                return Ok(());
            }
        }
...
async fn membership_without_optional_follow_up_is_treated_optional() -> anyhow::Result<()> {
    ...
    let label_id = match node.catalog.resolve(&ModelId::EntityId(model_id), "label").expect("label resolves after registration") {
        PropertyId::EntityId(id) => id,
        other => panic!("registered label resolved to non-entity property {other:?}"),
    };
    ...
    node.commit_remote_transaction(
        ...
        vec![proto::ModelContext::new(proto::ModelId::System(proto::SystemModel::ModelProperty), proto::Attested::opt(genesis, None))],
    )
    .await?;
...
fn membership_genesis(model: &EntityId, property: &EntityId) -> proto::Event {
    let backend = LWWBackend::new();
    backend.set(PropertyId::System(proto::SystemProperty::Model), Some(Value::EntityId(*model)));
    backend.set(PropertyId::System(proto::SystemProperty::Property), Some(Value::EntityId(*property)));
