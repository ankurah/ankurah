...
struct MockFilterable {
    collection: String,
    values: HashMap<ankql::ast::PropertyId, Value>,
}
impl MockFilterable {
...
fn fixture_prop(name: &str) -> ankql::ast::PropertyId {
    let mut bytes = [0u8; 32];
    let n = name.as_bytes();
    let len = n.len().min(32);
    bytes[..len].copy_from_slice(&n[..len]);
    ankql::ast::PropertyId::EntityId(ankurah::proto::EntityId::from_bytes(bytes))
}
