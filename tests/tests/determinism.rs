...
async fn cross_root_raw_state_does_not_substitute_property_ids() -> Result<()> {
    ...
    // Each root allocates its own model and property ids. An out-of-band state
    // copy preserves the foreign membership; it must never acquire the local
    // root's same-named model membership.
    ...
    assert!(
        matches!(ctx2.get::<RecordView>(record_id).await, Err(ankurah::error::RetrievalError::EntityNotFound(id)) if id == record_id),
        "a copied foreign state must not be readable through the local same-named model"
    );
    Ok(())
}
