...
async fn test_event_only_multi_event_wire_order_is_untrusted() -> Result<()> {
    ...
    let title_prop = common::resolved_prop(&server, Record::descriptor(), "title");
    let collection_id = common::resolved_model(&server, Record::descriptor());
    let artist_prop = common::resolved_prop(&server, Record::descriptor(), "artist");

    // Forge parent (writes artist) then child (writes title), and put the
    // CHILD first on the wire.
    let ev_parent = forge_title_event(collection_id, title_prop, rec_id, view.entity().head().clone(), "ignored");
    ...
    let ev_child = forge_title_event(collection_id, title_prop, rec_id, proto::Clock::from(vec![ev_parent.id()]), "t-child");
    ...
    let collection = common::collection_of::<Record>(&ctx_c).await?;
...
async fn test_event_only_unknown_entity_does_not_poison_batch() -> Result<()> {
    ...
    // about in the middle.
    let ev_a = forge_title_event(collection_id, title_prop, a_id, view_a.entity().head().clone(), "a1");
    let ev_b = forge_title_event(collection_id, title_prop, b_id, view_b.entity().head().clone(), "b1");
    let unknown_id = proto::EntityId::random();
    let ev_unknown =
        forge_title_event(collection_id, title_prop, unknown_id, proto::Clock::from(vec![proto::EventId::from_bytes([7u8; 32])]), "ghost");
