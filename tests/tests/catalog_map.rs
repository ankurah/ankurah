...
use ankurah::core::{
    ...
    storage::{CommitBatchOutcome, StorageEngine, StorageWriteBatch},
    value::Value,
};
...
struct FailingFetchEngine {
    inner: Arc<SledStorageEngine>,
    fail_model: Arc<Mutex<Option<ankurah::ModelId>>>,
}
...
impl StorageEngine for GatedCatalogFetchEngine {
    ...
    async fn append_events(&self, events: &[proto::Attested<proto::Event>]) -> Result<Vec<bool>, MutationError> {
    ...
    async fn fetch_states(
        &self,
        model: &ankurah::ModelId,
        selection: &ankql::ast::Selection,
    ) -> Result<Vec<proto::Attested<proto::EntityState>>, RetrievalError> {
        if ankurah::core::schema::is_catalog_collection(model) && self.gate.fetch_calls.fetch_add(1, Ordering::AcqRel) == 0 {
    ...
    async fn list_materializations(&self) -> Result<Vec<ankurah::ModelId>, RetrievalError> { self.inner.list_materializations().await }
...
#[async_trait]
impl StorageEngine for FailingFetchEngine {
    type Value = Vec<u8>;
    ...
    async fn fetch_states(
        ...
        if self.fail_model.lock().unwrap().as_ref() == Some(model) {
            return Err(RetrievalError::Other("injected activation fetch failure".to_owned()));
        }
        self.inner.fetch_states(model, selection).await
    }
...
impl StorageEngine for GatedListEngine {
    ...
    async fn delete_all(&self) -> Result<bool, MutationError> {
        let result = self.inner.delete_all().await;
        self.gate.delete_calls.fetch_add(1, Ordering::AcqRel);
    ...
    async fn list_materializations(&self) -> Result<Vec<ankurah::ModelId>, RetrievalError> {
        ...
        let collections = self.inner.list_materializations().await?;
...
fn forged_model_delta(collection: &str) -> (EntityId, proto::Attested<proto::EntityState>, proto::EntityDelta) {
    ...
    backend.set(PropertyId::System(proto::SystemProperty::Name), Some(Value::String("Stale generation".to_owned())));
...
async fn reset_invalidates_held_catalog_warm_generation() -> anyhow::Result<()> {
    ...
    assert_eq!(wait_resolve(&client, "album", "name").await, Some(replacement_name));
    assert!(client.catalog.model_by_label("stale_before_reset").is_none(), "forged old model must not enter the replacement map");
    assert!(client.storage.get_state(stale_id).await.is_err(), "forged old model must not be persisted after the reset clear");
...
async fn hard_reset_drains_in_flight_durable_catalog_warm() -> anyhow::Result<()> {
    ...
    // fence. Deletion is therefore impossible while model listing remains
...
async fn hard_reset_flushes_catalog() -> anyhow::Result<()> {
    ...
    let (registered_models, _, _) = expect_registered(client.request(server.id, &DEFAULT_CONTEXT, album_request()).await?);
    let album_model = registered_models[0].id;
    ...
    assert!(
        server.catalog.resolver().model_properties(&proto::ModelId::EntityId(album_model)).is_err(),
        "an unwarmed catalog must not masquerade as a complete zero-property model"
    );
...
async fn ephemeral_resolves_known_collection_offline_from_cache() -> anyhow::Result<()> {
    ...
    let album_model = client.catalog.model_id_for("album").expect("cached model identity survives disconnect");
    assert_eq!(client.catalog.resolve(&album_model, "name"), Some(PropertyId::EntityId(name_id)), "cache survives disconnect");
    assert!(client.catalog.resolve(&album_model, "name").is_some(), "resolution answers from the cache offline");
...
async fn schema_less_selection_update_warms_cold_catalog() -> anyhow::Result<()> {
    ...
    let album_model = server_ctx.model_id::<Album>().await?;
    let lq = EntityLiveQuery::new(&client, album_model, args, DEFAULT_CONTEXT)?;
    lq.wait_initialized().await;
    ...
    let album_model = client.catalog.model_id_for("album").expect("the warm must make the registered model resolvable");
    assert!(client.catalog.resolve(&album_model, "name").is_some(), "the warm must make the registered property resolvable");
...
async fn durable_activation_error_releases_initialization_waiters() -> anyhow::Result<()> {
    ...
    let node = Node::new_durable(
        Arc::new(FailingFetchEngine { inner: Arc::new(SledStorageEngine::new_test()?), fail_model: fail_model.clone() }),
        PermissiveAgent::new(),
    );
    node.system.create().await?;
    let model = ankurah::ModelId::EntityId(proto::EntityId::new());
    *fail_model.lock().unwrap() = Some(model);

    // The raw, property-free selection resolves without a catalog warm, then
    // the injected storage fetch failure reaches the post-resolution
    // activation-error branch.
    let args: MatchArgs = "true".try_into()?;
    let query = EntityLiveQuery::new(&node, model, args, DEFAULT_CONTEXT)?;
    ...
    query.error().with(|error| {
        ...
        assert!(error.to_string().contains("injected activation fetch failure"), "unexpected error: {error}");
    });
