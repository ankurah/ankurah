...
impl StorageEngine for GatedCatalogFetchEngine {
    ...
    async fn system_root_claim(&self) -> Result<Option<proto::SystemRootProof>, RetrievalError> {
        <SledStorageEngine as StorageEngine>::system_root_claim(self.inner.as_ref()).await
    }
...
fn forged_model_delta(collection: &str, system_root: EntityId) -> (EntityId, proto::StateWithGenesis, proto::EntityDelta) {
    ...
    let lww_operations = backend.to_operations().unwrap().expect("forged model has fields");
    ...
    // A structurally valid, genesis-derived identity: nothing about the
    // FORMAT may be rejectable, so staleness remains the only defense.
    let operations = proto::OperationSet(BTreeMap::from([("lww".to_owned(), lww_operations.clone())]));
    let genesis = proto::Event::genesis(model, Some(system_root), operations);
    let event_id = genesis.id();
    let entity_id = genesis.entity_id;
    let backend = LWWBackend::new();
    backend.apply_operations_with_event(&lww_operations, event_id.clone()).unwrap();
    ...
    let proof = proto::StateWithGenesis { genesis: proto::Attested::opt(genesis, None), state };
    (entity_id, proof, delta)
}
...
async fn reset_invalidates_held_catalog_warm_generation() -> anyhow::Result<()> {
    ...
    // Rejoin the same authority so this test can launch a visible
    // replacement generation. Hard reset closed the old sessions, so the
    // rejoin is a fresh handshake sharing the same gate (and counters).
    let _reconnection = GatedConnection::new_with_gate(&server, &client, gate.clone());
    client.system.wait_system_ready().await;
    ...
    let (stale_id, stale_state, stale_delta) =
        forged_model_delta("stale_before_reset", client.system.root_id().expect("client rejoined a system root"));
    client
...
async fn hard_reset_drains_in_flight_ephemeral_catalog_cache_fetch() -> anyhow::Result<()> {
    ...
    assert!(!client.catalog.is_catalog_ready());
    assert_eq!(client.catalog.counts(), (0, 0, 0));
    // Hard reset closed the old sessions; rejoin the same authority over a
    // fresh handshake.
    let _reconnection = LocalProcessConnection::new(&server, &client).await?;
    client.system.wait_system_ready().await;
...
async fn hard_reset_rejects_stale_schema_registration_response() -> anyhow::Result<()> {
    ...
    // may populate the replacement epoch normally. Hard reset closed the old
    // sessions, so the rejoin is a fresh handshake sharing the same gate.
    let _reconnection = GatedConnection::new_with_gate(&server, &client, gate.clone());
    client.system.wait_system_ready().await;
...
async fn durable_activation_error_releases_initialization_waiters() -> anyhow::Result<()> {
    ...
    storage
        .set_state(proto::Attested::opt(
            proto::EntityState {
                entity_id: proto::EntityId::from_bytes([0x77; 32]),
                model: proto::EntityId::from_bytes([0x78; 32]),
