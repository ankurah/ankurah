...
async fn server_refuses_commits_into_protected_collections() -> anyhow::Result<()> {
    ...
    for collection in PROTECTED {
        ...
        let event =
            proto::Event { entity_id: EntityId::new(), operations: proto::OperationSet(BTreeMap::new()), parent: proto::Clock::default() };
        let resp = client
            .request(
                server.id,
                &DEFAULT_CONTEXT,
                proto::NodeRequestBody::CommitTransaction {
                    ...
                    events: vec![proto::ModelContext::new(model, proto::Attested::opt(event, None))],
                },
            )
            .await?;
        match resp {
...
/// Unknown durable model identities never get storage: a fetch for one is
/// refused rather than creating a materialization on demand.
#[tokio::test]
async fn unknown_model_ids_get_no_storage() -> anyhow::Result<()> {
    let server = durable_sled_setup().await?;
    let client = ephemeral_sled_setup().await?;
    let _conn = LocalProcessConnection::new(&server, &client).await?;
    client.system.wait_system_ready().await;
    ...
    match resp {
        Ok(proto::NodeResponseBody::Error(e)) => assert!(e.contains("unknown") || e.contains("registered"), "unexpected error: {e}"),
        Err(e) => {
            let message = e.to_string();
            assert!(message.contains("unknown") || message.contains("registered"), "unexpected error: {message}");
        }
        Ok(other) => panic!("expected unknown-model refusal, got {other:?}"),
    }
    Ok(())
}
