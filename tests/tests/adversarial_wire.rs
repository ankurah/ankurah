...
use ankurah::{policy::DEFAULT_CONTEXT as c, Node, PermissiveAgent, View};
...
/// peer writes id-keyed operations (the id-keyed backend contract), and a
/// name-keyed forgery would simply never be read for a user property (a
/// backend read consults the id slot only, with no name fallback) -- these
/// tests pin containment and idempotency, so the forgeries must be
/// well-formed id-keyed events.
fn forge_title_event(entity_id: proto::EntityId, title_prop: proto::EntityId, parent: proto::Clock, title: &str) -> proto::Event {
    let backend = LWWBackend::new();
    backend.set(PropertyId::EntityId(title_prop), Some(Value::String(title.to_owned())));
    let ops = backend.to_operations().unwrap().expect("LWW backend with a write produces operations");
    proto::Event { entity_id, operations: proto::OperationSet::from_backends(BTreeMap::from([("lww".to_owned(), ops)])), parent }
}

fn event_only_item(event: proto::Event, model: proto::EntityId) -> proto::SubscriptionUpdateItem {
...
fn deliver(
    ...
    for item in &mut items {
        item.source_queries.insert(source_query);
    }
...
fn deliver_with_schema(
    ...
    schema: Vec<Attested<proto::EntityState>>,
) -> proto::NodeMessage {
    item.source_queries.insert(source_query);
    proto::NodeMessage::Update(proto::NodeUpdate {
        id: proto::UpdateId::new(),
...
/// Build the full name-keyed LWW state used by catalog entities. The schema
/// envelope is a cache warm, so this deliberately forges states directly
/// rather than committing catalog events on the sender.
fn forge_catalog_state(collection: &str, entity_id: proto::EntityId, fields: Vec<(&str, Option<Value>)>) -> Attested<proto::EntityState> {
    let backend = LWWBackend::new();
    for (name, value) in fields {
        backend.set(PropertyId::System(proto::SystemProperty::from_name(name).expect("catalog state field is a system property")), value);
    }
    ...
    let model = ankurah::core::schema::system_model_id(collection).expect("catalog collection has a system model");
...
async fn fixture() -> Result<Fixture> {
    ...
    let record_model_id = ctx_s.model_id::<Record>().await?;
    let record_model = *record_model_id.as_entity_id().expect("Record is a catalog-allocated model");
    let record_title = match server.catalog.resolve(&record_model_id, "title").expect("title registered with Record") {
        PropertyId::EntityId(id) => id,
        other => panic!("Record title must have an allocated property id, got {other:?}"),
    };
    let record_artist = match server.catalog.resolve(&record_model_id, "artist").expect("artist registered with Record") {
        PropertyId::EntityId(id) => id,
        other => panic!("Record artist must have an allocated property id, got {other:?}"),
    };
...
async fn stored_event_ids(node: &Node<SledStorageEngine, PermissiveAgent>, id: proto::EntityId) -> Result<Vec<proto::EventId>> {
    Ok(node.storage.dump_entity_events(id).await?.iter().map(|e| e.payload.id()).collect())
}
...
async fn stale_stream_is_rejected_before_schema_and_body() -> Result<()> {
    ...
    let forged = forge_title_event(rec_id, f.record_title, view.entity().head().clone(), "after");
    let forged_id = forged.id();
    let mut item = event_only_item(forged, f.record_model);
    item.source_queries.insert(proto::QueryId::new());

    let stale_model_id = proto::EntityId::new();
    let stale_schema = forge_catalog_state(
        "_ankurah_model",
        stale_model_id,
        vec![("label", Some(Value::String("stale_wire".to_owned()))), ("name", Some(Value::String("StaleWire".to_owned())))],
    );
    ...
    assert!(!stored_event_ids(&f.client, rec_id).await?.contains(&forged_id));
    assert!(f.client.catalog.model_by_label("stale_wire").is_none(), "a stale stream must not warm schema first");
    Ok(())
}

/// Shipped catalog states may refresh mutable metadata, but may never rewrite
/// allocation identity in the receiver's cache. Exercise the real connected,
/// recipient-checked, permissive-policy envelope seam for every immutable
/// field, then prove the intentionally mutable fields still update.
#[tokio::test]
async fn schema_envelope_preserves_immutable_catalog_identity() -> Result<()> {
    ...
    let probe_event =
        f.server.storage.dump_entity_events(probe_id).await?.into_iter().next().expect("probe record has a genesis event").payload;
    let probe_item = event_only_item(probe_event, f.record_model);
    let model = f.client.catalog.model_by_label("record").expect("record model warmed");
    ...
    let model_state = |collection: &str, name: &str| {
        forge_catalog_state(
            "_ankurah_model",
            model.id,
            vec![("label", Some(Value::String(collection.to_owned()))), ("name", Some(Value::String(name.to_owned())))],
        )
    };
    let property_state =
        |minted_for: Option<proto::EntityId>, backend: &str, value_type: &str, name: &str, target_model: Option<proto::EntityId>| {
            forge_catalog_state(
                "_ankurah_property",
                property.id,
                vec![
                    ...
                    ("target_model", target_model.map(Value::EntityId)),
                ],
            )
        };
    let membership_state = |model_id, property_id, optional| {
        forge_catalog_state(
    ...
    // Same model id, new unused collection.
    ship(model_state("wire_hijack", "Poisoned")).await?;
    assert_eq!(f.client.catalog.model_by_label("record"), Some(model.clone()));
    assert!(f.client.catalog.model_by_label("wire_hijack").is_none());

    // Each property allocation field is independently immutable. A poisoned
    // state is rejected wholesale, including its otherwise-mutable name.
    let property_attacks = [
        property_state(Some(proto::EntityId::new()), &property.backend, &property.value_type, "poisoned_name", property.target_model),
        property_state(None, &property.backend, &property.value_type, "poisoned_name", property.target_model),
        property_state(property.minted_for, "yrs", &property.value_type, "poisoned_name", property.target_model),
        property_state(property.minted_for, &property.backend, "i64", "poisoned_name", property.target_model),
    ];
    for attack in property_attacks {
        ship(attack).await?;
        assert_eq!(f.client.catalog.property_by_id(&property.id), Some(property.clone()));
    }

    // The membership id cannot move either endpoint.
    ship(membership_state(proto::EntityId::new(), membership.property, !membership.optional.unwrap_or(true))).await?;
    assert_eq!(f.client.catalog.membership(&membership.model, &membership.property), Some(membership.clone()));
    ship(membership_state(membership.model, proto::EntityId::new(), !membership.optional.unwrap_or(true))).await?;
    assert_eq!(f.client.catalog.membership(&membership.model, &membership.property), Some(membership.clone()));

    // Names, reference targets, and optionality are mutable metadata when all
    // immutable identity fields match.
    ship(model_state(&model.label, "RenamedRecord")).await?;
    assert_eq!(f.client.catalog.model_by_label("record").unwrap().name, "RenamedRecord");

    let target = proto::EntityId::new();
    ship(property_state(property.minted_for, &property.backend, &property.value_type, "renamed_title", Some(target))).await?;
    let updated_property = f.client.catalog.property_by_id(&property.id).unwrap();
    assert_eq!(updated_property.name, "renamed_title");
    assert_eq!(updated_property.target_model, Some(target));
    ship(property_state(property.minted_for, &property.backend, &property.value_type, "renamed_title", None)).await?;
    assert_eq!(f.client.catalog.property_by_id(&property.id).unwrap().target_model, None);

    let new_optional = !membership.optional.unwrap_or(true);
    ship(membership_state(membership.model, membership.property, new_optional)).await?;
    assert_eq!(f.client.catalog.membership(&membership.model, &membership.property).unwrap().optional, Some(new_optional));
...
async fn malformed_clock_identity_is_order_independent_end_to_end() -> Result<()> {
    ...
    let ev_b = forge_title_event(rec_id, f.record_title, head0.clone(), "b");
    let ev_c = {
        // Distinct ops so ev_c is a sibling, not a duplicate of ev_b.
        let backend = LWWBackend::new();
        backend.set(PropertyId::EntityId(f.record_artist), Some(Value::String("c-artist".to_owned())));
    ...
    reversed.reverse();
    let ev_merge_a = forge_title_event(rec_id, f.record_title, proto::Clock::from(two_ids.clone()), "merged");
    let ev_merge_b = forge_title_event(rec_id, f.record_title, proto::Clock::from(reversed), "merged");
    ...
    f.client.handle_message(f.delivery(vec![event_only_item(ev_merge_a.clone(), f.record_model)])).await?;
...
/// notifies, and no phantom resident or head movement is left for the forged
/// entity. The structurally valid, policy-accepted event remains in the
/// append-only event store so a subsequently received parent can ground it,
/// but it is not part of canonical entity state. The forger cannot manufacture
/// happened-before against history that does not exist.
#[tokio::test]
async fn forged_dangling_parent_is_contained() -> Result<()> {
    ...
    let ev_a = forge_title_event(a_id, f.record_title, view_a.entity().head().clone(), "a1");
    let id_ev_a = ev_a.id();
    let dangling = proto::Clock::from(vec![proto::EventId::from_bytes([0xAB; 32])]);
    let ev_b_forged = forge_title_event(b_id, f.record_title, dangling, "forged");
    let id_forged = ev_b_forged.id();

    // handle_message returns Ok; the per-item failure rides the ack path.
    f.client.handle_message(f.delivery(vec![event_only_item(ev_a, f.record_model), event_only_item(ev_b_forged, f.record_model)])).await?;

    // A applied; B is unchanged, its head did not move, and B's real state
    // survives. Event storage is intentionally independent of canonical-state
    // advancement, so the dangling event remains retrievable.
    ...
    assert!(stored_event_ids(&f.client, a_id).await?.contains(&id_ev_a), "A's valid event is stored");
    assert!(stored_event_ids(&f.client, b_id).await?.contains(&id_forged), "dangling event remains in the append-only store");
    Ok(())
}
...
async fn forged_extra_genesis_head_does_not_trigger_wholesale_adoption() -> Result<()> {
    ...
    let ev_b = forge_title_event(rec_id, f.record_title, head0.clone(), "child-b");
    let ev_x = {
        let backend = LWWBackend::new();
        backend.set(PropertyId::EntityId(f.record_artist), Some(Value::String("foreign-x".to_owned())));
        let ops = backend.to_operations().unwrap().expect("ops");
        let mut operations = proto::OperationSet::from_backends(BTreeMap::from([("lww".to_owned(), ops)]));
        operations.push(proto::Operation::Membership(proto::Membership::Add(proto::ModelId::EntityId(f.record_model))));
        proto::Event { entity_id: rec_id, operations, parent: proto::Clock::default() }
    };
    let id_b = ev_b.id();
    let id_x = ev_x.id();
...
fn declared_cycle_is_unconstructible_content_addressing() {
    ...
    let mk = |title: &str, parent: proto::Clock| forge_title_event(entity, title_prop, parent, title);
...
async fn fabricated_cycle_batch_is_contained() -> Result<()> {
    ...
    let ev1 = forge_title_event(rec_id, f.record_title, proto::Clock::from(vec![fake2]), "cycle-1");
    let ev2 = forge_title_event(rec_id, f.record_title, proto::Clock::from(vec![fake1]), "cycle-2");
    let id1 = ev1.id();
    let id2 = ev2.id();
...
async fn replay_flood_is_idempotent() -> Result<()> {
    ...
    let ev = forge_title_event(rec_id, f.record_title, view.entity().head().clone(), "t1");
    let id_ev = ev.id();

    // Flood: 10 identical single-event deliveries, none may error.
    for _ in 0..10 {
        f.client.handle_message(f.delivery(vec![event_only_item(ev.clone(), f.record_model)])).await?;
    }
...
async fn forged_second_genesis_rejected_on_durable_node() -> Result<()> {
    ...
    let before = stored_event_ids(&f.server, rec_id).await?;
    ...
    let mut alt = forge_title_event(rec_id, f.record_title, proto::Clock::default(), "ALT-GENESIS");
    alt.operations.push(proto::Operation::Membership(proto::Membership::Add(proto::ModelId::EntityId(f.record_model))));
    ...
    let after = stored_event_ids(&f.server, rec_id).await?;
...
/// and does not enter its head; the forged root is not adopted. The event has
/// already passed ingress policy and is blindly appended before the causal
/// comparison, so rejection affects canonical state, not append-only event
/// retention. (BFS may also pull the real genesis into storage as a side effect
...
async fn forged_second_genesis_rejected_on_ephemeral_node() -> Result<()> {
    ...
    let mut alt = forge_title_event(rec_id, f.record_title, proto::Clock::default(), "ALT-EPH");
    alt.operations.push(proto::Operation::Membership(proto::Membership::Add(proto::ModelId::EntityId(f.record_model))));
    let alt_id = alt.id();
    f.client.handle_message(f.delivery(vec![event_only_item(alt, f.record_model)])).await?;
    ...
    assert!(stored_event_ids(&f.client, rec_id).await?.contains(&alt_id), "rejected genesis remains in the append-only event store");
    Ok(())
}
...
async fn phantom_entity_is_evicted_on_failed_apply() -> Result<()> {
    ...
    let ev_unknown =
        forge_title_event(unknown_id, f.record_title, proto::Clock::from(vec![proto::EventId::from_bytes([7u8; 32])]), "ghost");
    let unknown_event_id = ev_unknown.id();

    f.client.handle_message(f.delivery(vec![event_only_item(ev_a, f.record_model), event_only_item(ev_unknown, f.record_model)])).await?;
    ...
    assert!(
        stored_event_ids(&f.client, unknown_id).await?.contains(&unknown_event_id),
        "the policy-accepted event remains durable even though no canonical entity state was created"
    );
    Ok(())
}
...
async fn oversized_event_batch_is_rejected() -> Result<()> {
    ...
    for i in 0..OVERSIZED {
        let ev = forge_title_event(rec_id, f.record_title, parent.clone(), &format!("flood-{i}"));
        parent = proto::Clock::from(vec![ev.id()]);
        events.push(ev);
    }

    let before = stored_event_ids(&f.client, rec_id).await?.len();
    ...
    let after = stored_event_ids(&f.client, rec_id).await?.len();
...
async fn equivocation_flood_antichain_is_bounded() -> Result<()> {
    ...
    let siblings: Vec<proto::Event> =
        (0..FLOOD).map(|i| forge_title_event(rec_id, f.record_title, head0.clone(), &format!("equiv-{i}"))).collect();
    for ev in siblings {
        f.client.handle_message(f.delivery(vec![event_only_item(ev, f.record_model)])).await?;
    }
