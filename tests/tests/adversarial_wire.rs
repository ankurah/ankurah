...
fn forge_title_event(entity_id: proto::EntityId, title_prop: proto::EntityId, parent: proto::Clock, title: &str) -> proto::Event {
    ...
    proto::Event { entity_id, operations: proto::OperationSet(BTreeMap::from([("lww".to_owned(), ops)])), parent }
}
...
fn deliver_with_schema(
    ...
    schema: Vec<proto::ModelContext<Attested<proto::EntityState>>>,
) -> proto::NodeMessage {
    item.source_queries.push(source_query);
    proto::NodeMessage::Update(proto::NodeUpdate {
        id: proto::UpdateId::new(),
...
fn forge_catalog_state(
    collection: &str,
    entity_id: proto::EntityId,
    fields: Vec<(&str, Option<Value>)>,
) -> proto::ModelContext<Attested<proto::EntityState>> {
    let backend = LWWBackend::new();
    ...
    proto::ModelContext::new(model, state)
}
...
async fn forged_second_genesis_rejected_on_durable_node() -> Result<()> {
    ...
    let alt = forge_title_event(rec_id, f.record_title, proto::Clock::default(), "ALT-GENESIS");
    ...
    let result = f
        ...
        .commit_remote_transaction(
            ...
            vec![proto::ModelContext::new(proto::ModelId::EntityId(f.record_model), Attested::opt(alt, None))],
        )
        .await;
...
async fn forged_second_genesis_rejected_on_ephemeral_node() -> Result<()> {
    ...
    let alt = forge_title_event(rec_id, f.record_title, proto::Clock::default(), "ALT-EPH");
    let alt_id = alt.id();
