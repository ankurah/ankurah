...
async fn test_backend_error_handling() -> Result<()> {
    ...
    let invalid_state = State { state_buffers: StateBuffers(state_buffers), head: Default::default(), memberships: Default::default() };
...
///
/// The test uses `commit_transaction` to re-deliver the event, which is
...
async fn test_redelivery_of_ancestor_event_is_noop() -> Result<()> {
    ...
    let album_model = node.catalog.model_id_for("album")?.expect("Album registered by create");
    ...
    // Step 4: Re-deliver event B via commit_transaction
    // Wrap the raw event in Attested (no attestations needed for PermissiveAgent)
    let attested_events: Vec<_> =
