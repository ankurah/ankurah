...
async fn catalog_values(
    ...
    // backend values are all `System { name }`; project to their name for the
    // by-string lookups the assertions use.
    Ok(LWWBackend::from_state_buffer(&buffer)?
        .property_values()
        .into_iter()
        .map(|(k, v)| match k {
            PropertyId::System { name } => (name, v),
...
fn catalog_lww_event(collection: &str, id: EntityId, parent: proto::Clock, fields: Vec<(&str, Value)>) -> proto::Event {
    ...
    for (name, value) in fields {
        backend.set(PropertyId::System { name: name.to_string() }, Some(value));
    }
    let operations = backend.to_operations().unwrap().expect("catalog event has fields");
    proto::Event {
        model: proto::ModelId::WellKnown(
            proto::WellKnownModel::from_collection(collection).expect("catalog collection is a well-known model"),
        ),
        entity_id: id,
...
async fn stored_bound_metric_value(
    ...
    Ok(backend.entry(&PropertyId::EntityId(property.to_ulid())).flatten())
}
...
async fn stored_bound_text(
    ...
    Ok(backend.get_string(&PropertyId::EntityId(property.to_ulid())))
}
...
async fn offline_ordinary_field_does_not_capture_explicitly_shared_same_name() -> anyhow::Result<()> {
    ...
    server
        .commit_remote_transaction(&DEFAULT_CONTEXT, proto::TransactionId::new(), vec![proto::Attested::opt(shared_property, None)])
        .await?;
