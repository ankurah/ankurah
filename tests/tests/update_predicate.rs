...
async fn test_predicate_update() -> Result<()> {
    ...
    // Should have Bravo, Charlie (sort for deterministic order - update_selection has to fetch, and entity ids do not sort by creation time)
...
async fn test_predicate_update_inter_node() -> Result<()> {
    ...
    // Should now have all 3 albums (sort for deterministic order - update_selection has to fetch, and entity ids do not sort by creation time)
    assert_eq!(albums.ids_sorted(), sorted![a_id, b_id, c_id]);
    assert_eq!(watcher.drain_sorted(), vec![sortby_t0![(a_id, ChangeKind::Initial), (b_id, ChangeKind::Initial)]]);
