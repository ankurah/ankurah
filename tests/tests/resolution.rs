...
async fn resolution_binds_names_and_fails_closed() -> anyhow::Result<()> {
    ...
    let collection = server.catalog.model_id_for("album").unwrap().expect("album model registered");

    let resolved = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("name = 'x'")?)?;
    ...
    let resolved = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("payload.meta.genre = 'jazz'")?)?;
    match first_expr(&resolved) {
        Expr::Path(ident) => {
    ...
    let resolved = resolve_selection(
        &collection,
        server.catalog.as_ref(),
        ankql::parser::parse_selection(&format!("id = '{}'", entity_id_literal()))?,
    )?;
    match first_expr(&resolved) {
    ...
    let resolved = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("album.name = 'x'")?)?;
    match first_expr(&resolved) {
        Expr::Path(ident) => {
    ...
    let err = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("bogus = 1")?).unwrap_err();
...
async fn resolution_follows_renames_to_the_same_id() -> anyhow::Result<()> {
    ...
    let before = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("name = 'x' ORDER BY name")?)?;
    ...
    let after = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("title = 'x'")?)?;
    ...
    assert!(resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("name = 'x'")?).is_err());
...
async fn order_by_resolves_fail_closed() -> anyhow::Result<()> {
    ...
    let known = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("true ORDER BY name")?)?;
    assert!(order_property_id(&known.order_by.as_ref().unwrap()[0]).is_some());
    let pseudo = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("true ORDER BY id")?)?;
    assert_eq!(order_property_id(&pseudo.order_by.as_ref().unwrap()[0]), None);
    let err = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("true ORDER BY bogus")?).unwrap_err();
    ...
    let resolved = resolve_selection(&collection, server.catalog.as_ref(), qualified)?;
...
async fn subpath_order_keys_and_id_subpaths_are_refused() -> anyhow::Result<()> {
    ...
    for steps in [&["payload", "meta"][..], &["album", "payload", "meta"][..], &["id", "x"][..], &["album", "id", "x"][..]] {
        let err = resolve_selection(&collection, server.catalog.as_ref(), order(steps)).unwrap_err();
    ...
    assert!(resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("payload.meta.genre = 'jazz'")?).is_ok());
    for query in ["id.foo = 'x'", "album.id.foo = 'x'"] {
        let err = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection(query)?).unwrap_err();
        assert!(err.to_string().contains("has no subfields"), "query {query:?}: {err}");
    }
    let err = resolve_selection(&catalog_collection, server.catalog.as_ref(), ankql::parser::parse_selection("id.foo = 'x'")?).unwrap_err();
...
async fn systemize_strips_the_collection_qualifier() -> anyhow::Result<()> {
    ...
    let err = resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("_ankurah_model.name.x = 'y'")?)
        .unwrap_err();
    ...
    let err =
        resolve_selection(&collection, server.catalog.as_ref(), ankql::parser::parse_selection("_ankurah_model.id.x = 'y'")?).unwrap_err();
    assert!(err.to_string().contains("has no subfields"), "got: {err}");
    ...
    let resolved = resolve_selection(&collection, server.catalog.as_ref(), order(&["_ankurah_model", "name"]))?;
    let items = resolved.order_by.expect("order_by present");
    assert_eq!(items[0].path.property_id(), PropertyId::System(proto::SystemProperty::Name));
    assert!(items[0].path.subpath.is_empty());
    let err = resolve_selection(&collection, server.catalog.as_ref(), order(&["_ankurah_model", "name", "x"])).unwrap_err();
    assert!(err.to_string().contains("not sortable"), "got: {err}");
