...
//! drifted-but-castable type operates through `Value::cast_to`: generated
//! accessors canonicalize values before staging them (and commit repeats the
//! check for low-level staging), its getters cast canonical values back out to
//! the compiled type, and comparison literals canonicalize at resolution so
//! predicate evaluation matches the stored type. A value the canonical type
//! cannot represent fails its own mutation at the writer.
//!
...
fn order_property_id(item: &ankql::ast::OrderByItem) -> Option<ankurah::EntityId> {
    match &item.key {
...
fn assert_resolved_f64_comparison(selection: &ankql::ast::Selection, expected: f64) {
    ...
    match &selection.predicate {
        Predicate::Comparison { left, right, .. } => {
            ...
            assert!(
                matches!(right.as_ref(), Expr::Literal(Value::F64(value)) if *value == expected),
...
async fn castable_drift_writes_and_reads_through_the_canonical_type() -> Result<()> {
    ...
    // v2 (compiled i64) creates another film: the I64 canonicalizes to I32
    // before it enters the backend's uncommitted set.
    let v2_film_id = {
        let trx = ctx.begin();
    ...
    let state = node.storage.get_state(v2_film_id).await?;
    ...
    // A value the canonical type cannot represent fails before staging, at the
    // writer: 5_000_000_000 does not fit i32.
    let trx = ctx.begin();
    let err = trx.create(&v2::Film { year: 5_000_000_000 }).await.expect_err("an i64 value exceeding canonical i32 must not stage");
...
async fn distinct_schema_for_ensured_collection_still_runs_compatibility_gate() -> Result<()> {
    ...
    let query_error = match ctx.query_wait::<incompatible_backend::ProfileView>("name = 'canonical yrs'").await {
        Ok(_) => panic!("query_wait unexpectedly admitted an incompatible schema"),
        Err(error) => error,
    };
    assert!(query_error.to_string().contains("not castable"), "the incompatible query must fail registration, got: {query_error}");
...
async fn ill_typed_payload_is_contained_at_every_read_boundary() -> Result<()> {
    ...
    let film_model = ctx.model_id::<v1::Film>().await?;
    let mut state = node.storage.get_state(film_id).await?;
    let forged = {
        let b = ankurah::core::property::backend::LWWBackend::new();
        b.set(ankql::ast::PropertyId::EntityId(year_id), Some(Value::String("abc".into())));
    ...
    let expected_head = state.payload.state.head.clone();
    let outcome = node
        .storage
    ...
    let resolved = ankql::parser::parse_selection("year = 2001")?.resolve_names(&film_model, node.catalog.resolver())?;
    assert!(!evaluate_predicate(view.entity(), &resolved.predicate)?, "junk must not match its pre-forge value");
    let resolved = ankql::parser::parse_selection("year = 5")?.resolve_names(&film_model, node.catalog.resolver())?;
