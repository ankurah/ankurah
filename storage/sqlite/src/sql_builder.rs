...
impl SqlBuilder {
    fn new() -> Self { Self { sql: String::new(), params: Vec::new(), fields: Vec::new(), table_name: None, column_map: BTreeMap::new() } }
    ...
    pub(crate) fn build(self) -> Result<(String, Vec<rusqlite::types::Value>), SqlGenerationError> {
    ...
    #[allow(dead_code)]
    fn build_where_clause(self) -> (String, Vec<rusqlite::types::Value>) { (self.sql, self.params) }
    ...
    /// path steps. Shared by the `Expr::Path` and `Expr::PropertyPath` arms so both
    /// render identically.
...
mod tests {
    ...
    fn test_identifier_unmapped_property_errors() {
        use ankql::ast::{ComparisonOperator, Expr, Predicate, PropertyPath};
        use ankurah_core_types::Value;
        use ankurah_core::EntityId;
