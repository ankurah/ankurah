...
impl StorageEngine for SqliteStorageEngine {
    ...
    async fn commit_batch(&self, batch: StorageWriteBatch) -> Result<CommitBatchOutcome, MutationError> {
        ...
        for write in &batch.entities {
            ...
            models_by_entity.insert(entity_id, write.state.payload.state.memberships.clone());
        }
            ...
            let attempt = conn
                .with_connection_mut(move |c| {
                    ...
                    for (_, write) in &ordered {
                        let entity_id = write.state.payload.entity_id;
                        let entity_key = entity_id.to_base64();
                        let target = &write.state.payload.state.memberships;
                        let previous = memberships_from_sqlite_connection(&tx, &entity_key)?;
                        if !previous.is_subset(target) {
                            return Err(SqliteError::CorruptRecord(format!(
                        ...
                        let models: Vec<ModelId> = target.iter().copied().collect();
                        for model in &models {
    ...
    async fn fetch_states(&self, selection: &ankql::ast::Selection) -> Result<Vec<Attested<EntityState>>, RetrievalError> {
        ...
        for property in effective_selection.referenced_properties() {
            ...
            let value_type = if let Some(resolver) = resolver.as_deref() {
                resolver.property_value_type(&property).map_err(|error| RetrievalError::Other(error.to_string()))?
            } else {
                assigned
                    .get(&property)
                    .and_then(|column| physical_types.get(column))
                    .copied()
                    .ok_or_else(|| RetrievalError::Other(format!("SQLite has no comparison type for property {property}")))?
            };
...
async fn load_states(conn: &PooledConnection, ids: &[EntityId]) -> Result<Vec<Attested<EntityState>>, RetrievalError> {
    ...
    for (id, state_buffer, head, attestations) in raw {
        ...
        let membership_key = id.clone();
        let memberships =
            conn.with_connection(move |c| memberships_from_sqlite_connection(c, &membership_key)).await.map_err(RetrievalError::storage)?;
        by_id.insert(
            entity_id,
            Attested {
                payload: EntityState {
...
mod tests {
    ...
    async fn commit_canonical_state(engine: &SqliteStorageEngine, expected_head: Clock, state: Attested<EntityState>) {
        let outcome = engine.commit_batch(StorageWriteBatch::new(vec![PreparedEntityWrite::new(expected_head, state)])).await.unwrap();
