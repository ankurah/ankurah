...
impl SqliteBucket {
    ...
    /// Check if a column exists (without cloning)
    fn has_column(&self, name: &str) -> bool {
        let columns = self.columns.read().expect("RwLock poisoned");
        columns.iter().any(|c| c.name == name)
    }

    /// Check which columns from a list are missing from the cache
    fn filter_missing_columns(&self, columns: &[String]) -> Vec<String> {
        let cached = self.columns.read().expect("RwLock poisoned");
        columns.iter().filter(|col| !cached.iter().any(|c| &c.name == *col)).cloned().collect()
    }
...
impl StorageCollection for SqliteBucket {
    ...
    async fn fetch_states(&self, selection: &ankql::ast::Selection) -> Result<Vec<Attested<EntityState>>, RetrievalError> {
        ...
        // Pre-filter selection based on cached schema
        let referenced = selection.referenced_columns();

        // Check if any referenced columns are unknown to cache
        let has_unknown = {
            let columns = self.columns.read().expect("RwLock poisoned");
            referenced.iter().any(|col| !columns.iter().any(|c| &c.name == col))
        }; // Lock released here

        if has_unknown {
            debug!("Unknown columns in query, refreshing schema cache");
            self.rebuild_columns_cache(&conn).await?;
        }

        // Find missing columns (not in schema even after refresh)
        let missing = self.filter_missing_columns(&referenced);

        let effective_selection = if missing.is_empty() {
            selection.clone()
        } else {
            debug!("Columns {:?} don't exist, treating as NULL", missing);
