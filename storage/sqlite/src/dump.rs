...
impl StorageDump for SqliteStorageEngine {
    ...
    async fn dump(&self) -> Result<Self::DumpStream, RetrievalError> {
        let (event_collections, state_collections) = {
            ...
            conn.with_connection(discover_collections_from_schema).await?
        };
...
fn event_page(
    ...
    let (query, arguments): (String, Vec<rusqlite::types::Value>) = if let Some(after) = after {
        (
            format!("SELECT id, entity_id, body, parent, attestations FROM {table} WHERE id > ? ORDER BY id LIMIT ?"),
            vec![after.to_owned().into(), PAGE_SIZE.into()],
        )
    } else {
        (format!("SELECT id, entity_id, body, parent, attestations FROM {table} ORDER BY id LIMIT ?"), vec![PAGE_SIZE.into()])
    };
    ...
    while let Some(row) = rows.next()? {
        ...
        let event = Attested::<Event>::from_parts(entity_id, collection.clone(), EventFragment { body, parent, attestations });
...
fn state_page(
    ...
    while let Some(row) = rows.next()? {
        ...
        let attestations = bincode::deserialize::<AttestationSet>(&row.get::<_, Vec<u8>>(4)?)?;
        last = Some(stored_id);
        let fragment = StateFragment { state: State { state_buffers, memberships, head }, attestations };
        page.push(StorageDumpItem::State(Attested::<EntityState>::from_parts(entity_id, collection.clone(), fragment)));
    }
    Ok((last, page))
}
