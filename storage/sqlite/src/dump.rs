...
fn state_page(
    ...
    let (query, arguments): (String, Vec<rusqlite::types::Value>) = if let Some(after) = after {
        (
            format!("SELECT id, state_buffer, head, attestations FROM {table} WHERE id > ? ORDER BY id LIMIT ?"),
            vec![after.to_owned().into(), PAGE_SIZE.into()],
        )
    } else {
        (format!("SELECT id, state_buffer, head, attestations FROM {table} ORDER BY id LIMIT ?"), vec![PAGE_SIZE.into()])
    };
    let mut statement = conn.prepare(&query)?;
    let mut rows = statement.query(rusqlite::params_from_iter(arguments))?;
    let mut last = None;
