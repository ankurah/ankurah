...
impl SledStorageCollectionInner {
    ...
    fn set_state_blocking(&self, state: Attested<EntityState>) -> Result<bool, MutationError> {
        ...
        for (property, opt_val) in entity.values() {
            if let Some(val) = opt_val {
                mat.push((self.database.property_manager.get_property_id(&property.to_string())?, val));
            }
        }
    ...
    fn fetch_states_blocking(&self, selection: ankql::ast::Selection<EngineColumns>) -> Result<Vec<Attested<EntityState>>, RetrievalError> {
        ...
        let plan = plans.into_iter().next().ok_or_else(|| RetrievalError::storage("No plan generated"))?;
