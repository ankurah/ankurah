...
pub struct SledMaterializationScanner {
    iter: SledMaterializationIter,
}

impl SledMaterializationScanner {
...
impl Unpin for SledMaterializationScanner {}

impl Stream for SledMaterializationScanner {
    ...
    fn poll_next(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let map = property_values.into_iter().collect();
...
impl<S: EntityIdStream> SledMaterializationLookup<S> {
    ...
    pub fn new(tree: &sled::Tree, stream: S) -> Self { Self { tree: tree.clone(), stream } }
}
