    ...
    fn commit_batch_blocking(
        ...
        loop {
            ...
            for (original_index, write) in batch.entities.iter().cloned().enumerate() {
                ...
                for model in &state_fragment.state.memberships {
                    ...
                    prepared_materializations
                        .insert((entity_id, *model), PreparedSledMaterialization { model: *model, tree, values, encoded, indexes });
                }
            ...
            let attempt: Result<StorageCommitResult, TransactionError<SledBatchAbort>> = trees.as_slice().transaction(|transactional| {
                ...
                for prepared in &prepared_entities {
                    ...
                    let previous_models: BTreeSet<ModelId> = associations
                        ...
                        .transpose()?
                        .unwrap_or_default();
                    let models = write.state.payload.state.memberships.clone();
                    if !previous_models.is_subset(&models) {
                        return Err(ConflictableTransactionError::Abort(SledBatchAbort::Invalid(format!(
                    ...
                    results.push((
                        ...
                        CommittedEntityWrite {
                            ...
                            associations_added: models.difference(&previous_models).copied().collect(),
