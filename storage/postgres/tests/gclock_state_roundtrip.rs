//! GClock pin (iii), postgres arm (D2 M4, plan REV 5 section K home 3): the
//! per-tip head generations survive set_state and rehydration through the
//! entity record (a generations array column parallel to the head array),
//! so a restarted node reconstitutes the materialization WITHOUT reading
//! events. Exercises both row-decode paths (get_state and fetch_states).
//! Includes a multi-tip head and the u32::MAX saturation sentinel.
