...
impl From<SqlGenerationError> for RetrievalError {
    fn from(err: SqlGenerationError) -> Self { RetrievalError::storage(err) }
}
...
impl SqlBuilder {
    ...
    pub fn order_by_item(&mut self, order_by: &OrderByItem<EngineColumns>) -> Result<(), SqlGenerationError> {
        if !order_by.path.subpath.is_empty() {
            return Err(SqlGenerationError::UnsupportedExpression("JSON paths are not supported in ORDER BY"));
        }
...
mod tests {
    ...
    #[test]
    fn order_by_rejects_json_subpaths() {
        let mut path = ColumnPath::simple("data");
        path.subpath.push("name".into());
        let item = OrderByItem { path, direction: OrderDirection::Asc };
        let mut sql = SqlBuilder::with_fields(vec!["id"]);
        assert!(matches!(sql.order_by_item(&item), Err(SqlGenerationError::UnsupportedExpression(_))));
    }
    ...
    fn test_selection_with_order_by() -> Result<()> {
        ...
        let selection = Selection {
            ...
            order_by: Some(vec![OrderByItem { path: ColumnPath::simple("created_at"), direction: OrderDirection::Desc }]),
            limit: None,
        };
    ...
    fn test_selection_with_order_by_and_limit() -> Result<()> {
        ...
        let selection = Selection {
            predicate: base_selection.predicate,
            order_by: Some(vec![
                OrderByItem { path: ColumnPath::simple("priority"), direction: OrderDirection::Desc },
                OrderByItem { path: ColumnPath::simple("created_at"), direction: OrderDirection::Asc },
            ]),
            limit: Some(5),
        };
    ...
    mod jsonb_sql_tests {
        ...
        fn test_two_step_json_path() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""licensing"->'territory' = $1"#);
            Ok(())
        }
        ...
        fn test_three_step_json_path() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""licensing"->'rights'->'holder' = $1"#);
            Ok(())
        }
        ...
        fn test_four_step_json_path() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""a"->'b'->'c'->'d' = $1"#);
            Ok(())
        }
        ...
        fn test_json_path_with_numeric_comparison() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""data"->'count' > $1"#);
            Ok(())
        }
        ...
        fn test_mixed_simple_and_json_paths() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""name" = $1 AND "data"->'status' = $2"#);
            Ok(())
        }
        ...
        fn test_json_path_escaping() -> Result<()> {
            ...
            let mut sql = SqlBuilder::new();
            let path = ColumnPath::new("data", vec!["it's".to_string()]);
        ...
        fn test_json_path_with_boolean() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""data"->'active' = $1"#);
            Ok(())
        }
        ...
        fn test_json_path_with_float() -> Result<()> {
            ...
            assert_eq!(sql_string, r#""data"->'score' >= $1"#);
            Ok(())
        }
    }
