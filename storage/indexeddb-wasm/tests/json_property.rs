...
pub fn test_json_path_planner_generates_sub_path() {
    ...
    // The planner reads this engine's fields, and the lowering renders a
    // resolved property id into one; bind `licensing` to a forged id the way
    // the fetch path binds names to catalog ids.
    ...
    // The planner only ever sees lowered selections. This one is lowered
    // without the engine's collection scoping, so the sub-path keypart under
    // test is the first one.
    let selection = ankurah_storage_common::lower_selection(&selection, &|path: &ankql::ast::PropertyPath| {
        ankurah_storage_common::ColumnPath::new(path.property_id().to_string(), path.subpath.clone())
    });
