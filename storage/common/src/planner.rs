...
impl Planner {
    ...
    ///
    /// Physical fields default to the literal's type (and ORDER BY fields to
    /// String collation). Engines that know their materialized field types
    /// should call [`Self::plan_with_types`] so predicate bounds and sort keys
    /// are checked in each property's canonical value type.
    ...
    /// - `physical_type_of` resolves a physical column to the property's
    ///   canonical value type. Predicate literals are cast through it again at
    ///   this execution boundary before becoming index bounds, and ordered
    ///   scans use it for collation. `None` retains the literal/String fallback
    ///   used by unresolved planner tests and synthetic engine fields.
    ...
    pub fn plan_with_types(
        ...
        primary_key: &str,
        physical_type_of: &dyn Fn(&str) -> Option<ValueType>,
        column_of: ColumnOf,
    ) -> Vec<Plan> {
        ...
        let (equalities, inequalities) =
            self.categorize_conjuncts_excluding_primary_key(&conjuncts, primary_key, physical_type_of, column_of);
        ...
        {
            if let Some(plan) =
                self.build_order_first_plan(&equalities, &inequalities, order_by, &conjuncts, primary_key, physical_type_of, column_of)
            {
                plans.push(plan);
            }
    ...
    fn categorize_conjuncts_excluding_primary_key(
        ...
        for conjunct in conjuncts {
            if let Some((field, op, value)) = self.extract_comparison(conjunct, column_of) {
                ...
                // Never trust origin-time AST normalization as proof. Cast the
                // threshold again against the engine's physical field type
                // immediately before it becomes an index key/bound. JSON
                // subpaths are encoded in the JSON collation domain rather
                // than the base property's declared type. An uncastable value
                // remains a residual predicate; it must not seed a false index
                // range that could exclude matching rows.
                let target = if field.contains('.') { Some(ValueType::Json) } else { physical_type_of(&field) };
                let value = match target {
                    Some(target) => match value.cast_to(target) {
                        Ok(value) => value,
                        Err(_) => continue,
                    },
                    None => value,
...
mod tests {
    ...
    mod primary_key_keypart_tests {
        ...
        fn resolved_pk_order_by_never_keyparted() {
            use ankql::ast::{Expr, OrderByItem, OrderDirection, OrderKey, Predicate, PropertyId, PropertyPath, Selection, Value};
            ...
            let status_id = EntityId::from_bytes([0x51; 16]);
            let selection = Selection {
                predicate: Predicate::Comparison {
                    left: Box::new(Expr::PropertyPath(PropertyPath::registered(status_id, "status", vec![]))),
            ...
            let column_of = |pid: &PropertyId| -> Option<String> {
                match pid {
                    ...
                    PropertyId::EntityId(id) if *id == status_id => Some("c_status".to_string()),
                    _ => None,
                }
            };
            let planner = Planner::new(PlannerConfig::full_support());
        ...
        #[test]
        fn execution_boundary_recasts_resolved_predicate_for_index() {
            use ankql::ast::{Expr, Predicate, PropertyId, PropertyPath, Selection, Value};
            use ankurah_core::EntityId;

            let property = EntityId::from_bytes([0x61; 16]);
            let property_id = PropertyId::EntityId(property);
            let selection = Selection {
                predicate: Predicate::Comparison {
                    left: Box::new(Expr::PropertyPath(PropertyPath::registered(property, "count", vec![]))),
                    operator: ComparisonOperator::Equal,
                    // Deliberately unnormalized, as if received from an
                    // untrusted/programmatic AST producer.
                    right: Box::new(Expr::Literal(Value::String("42".to_string()))),
                },
                order_by: None,
                limit: None,
            };
            let column_of = |id: &PropertyId| (*id == property_id).then(|| "c_count".to_string());
            let physical_type_of = |column: &str| (column == "c_count").then_some(ValueType::I32);

            let plans = Planner::new(PlannerConfig::full_support()).plan_with_types(&selection, "id", &physical_type_of, &column_of);
            let Plan::Index { index_spec, bounds, .. } = &plans[0] else { panic!("castable equality should produce an index plan") };
            assert_eq!(index_spec.keyparts, vec![asc!("c_count", ValueType::I32)]);
            assert_eq!(
                bounds.keyparts[0],
                KeyBoundComponent {
                    column: "c_count".to_string(),
                    low: Endpoint::incl(Value::I32(42)),
                    high: Endpoint::incl(Value::I32(42)),
                }
            );

            let uncastable = Selection {
                predicate: Predicate::Comparison {
                    left: Box::new(Expr::PropertyPath(PropertyPath::registered(property, "count", vec![]))),
                    operator: ComparisonOperator::Equal,
                    right: Box::new(Expr::Literal(Value::String("not-an-i32".to_string()))),
                },
                order_by: None,
                limit: None,
            };
            let plans = Planner::new(PlannerConfig::full_support()).plan_with_types(&uncastable, "id", &physical_type_of, &column_of);
            assert_eq!(plans.len(), 1);
            assert!(matches!(&plans[0], Plan::TableScan { remaining_predicate, .. } if remaining_predicate == &uncastable.predicate));
        }
    }
                ...
                vec![
                    ...
                    Plan::Index {
                        ...
                        remaining_predicate: Predicate::Comparison {
                            ...
                            right: Box::new(Expr::Literal(ankql::ast::Value::String("2001".to_string()))),
                        },
