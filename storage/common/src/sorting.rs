...
fn extract_partition_key<T: ValueLookup<EngineColumns>>(
    item: &T,
    presort: &[ankql::ast::OrderByItem<EngineColumns>],
) -> Vec<Option<Value>> {
...
where
    S: Stream + Unpin,
    S::Item: ValueLookup<EngineColumns>,
{
    type Item = S::Item;
...
impl<T: ValueLookup<EngineColumns>> PartialEq for HeapItem<T> {
    fn eq(&self, other: &Self) -> bool { self.cmp(other) == Ordering::Equal }
}

impl<T: ValueLookup<EngineColumns>> Eq for HeapItem<T> {}

impl<T: ValueLookup<EngineColumns>> PartialOrd for HeapItem<T> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> { Some(self.cmp(other)) }
}

impl<T: ValueLookup<EngineColumns>> Ord for HeapItem<T> {
...
mod tests {
    ...
    impl ValueLookup<EngineColumns> for TestItem {
